...
const ProvinceTicketPage = ({ province: provinceProp }) => {
  ...
  const { stadiums, stadiumImageSchedules, specialMatches, dailyImages, matchCards, upcomingFightsBackground, dbLoaded } = useDatabase(language);
  ...
  React.useEffect(() => {
    if (provinceData && isValidProvince) {
      ...
      document.title = `${name} Muay Thai Tickets - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
    }
