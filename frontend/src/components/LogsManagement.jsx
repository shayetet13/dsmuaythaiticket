...
const LogsManagement = () => {
  ...
  const fetchStadiums = async () => {
    try {
      // ใช้ API เดียวกับ Manage Stadiums เพื่อให้รายการสนามตรงกัน
      const response = await fetch(`${API_URL}/images/stadiums`);
  ...
  const getTicketDisplay = (booking) => {
    if (booking.ticket_id && booking.ticket_type) {
      const typeLabel = booking.ticket_type === 'regular' ? 'ตั๋วปกติ' : 'ตั๋วพิเศษ';
      const name = booking.ticket_name || null;
      return { name: name || 'ไม่ระบุชื่อ', typeLabel };
    }
    if (booking.zone_name) {
      return { name: booking.zone_name, typeLabel: null };
    }
    return { name: 'ไม่ระบุ', typeLabel: null };
  };
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {filteredBookings.map((booking) => {
              const ticketDisplay = getTicketDisplay(booking);
              return (
              <div key={booking.id} className="bg-gray-800 rounded-lg border border-gray-700 p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-start gap-2">
                    <span className="text-gray-400 flex-shrink-0">ตั๋ว:</span>
                    <div>
                      <div className="text-white">{ticketDisplay.name}</div>
                      {ticketDisplay.typeLabel && (
                        <div className="text-gray-400 text-xs mt-0.5">{ticketDisplay.typeLabel}</div>
                      )}
                    </div>
                  </div>
                  <div>
