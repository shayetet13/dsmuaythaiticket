...
const OptimizedImage = ({
  ...
  const handleError = (e) => {
    console.log('[OptimizedImage] Image load error:', imgSrc);
    
    if (!hasError) {
      // Try fallback image
      if (imgSrc !== fallback) {
        console.log('[OptimizedImage] Using fallback:', fallback);
        setImgSrc(fallback);
        setHasError(true);
      }
    }
...
export const ProgressiveImage = ({ 
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      console.log('[ProgressiveImage] High-res image failed to load');
      setIsLoading(false);
    };
  }, [highResSrc]);
