...
const HeroSection = ({ heroImage, t }) => {
  ...
  const imageSrc = hasValidImage ? heroImage.image : (heroImage?.fallback || '/images/highlights/World class fighters.jpg');
  const imageAlt = heroImage?.alt || 'Muay Thai';
  const fallbackSrc = heroImage?.fallback || '/images/highlights/World class fighters.jpg';
  ...
  useEffect(() => {
    if (imageSrc) {
      preloadImage(imageSrc).catch(err => {
        console.log('[HeroSection] Failed to preload hero image:', err);
      });
    }
  }, [imageSrc]);
    ...
    <section id="home" className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        <img
          ...
          onError={(e) => {
            console.log('[HeroSection] Image load error, using fallback:', imageSrc);
          ...
          onLoad={() => {
            console.log('[HeroSection] Image loaded successfully:', imageSrc);
          }}
        />
