...
import UpcomingFightBanner from './UpcomingFightBanner';
...
const StadiumUpcomingPage = ({ stadiumIdFromPath }) => {
  ...
  const { stadiums, stadiumImageSchedules, specialMatches, dailyImages, matchCards, upcomingFightsBackground, stadiumHighlightImages, stadiumDescriptions, upcomingFightBanners, dbLoaded } = useDatabase(language);
  ...
  React.useEffect(() => {
    if (stadiumData?.name) {
      ...
      document.title = `${baseName} Stadium Ticket - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
    } else {
      ...
      <div className="pt-20">
        {!dbLoaded || stadiums.length === 0 ? (
          ...
          <>
            ...
            />
            {(() => {
              const banner = upcomingFightBanners.find(b => b.targetId === stadiumId);
              if (!banner || !banner.isVisible) return null;
              // Auto mode doesn't need eventName; manual mode does
              if (!banner.isAuto && !banner.eventName) return null;
              return (
                <UpcomingFightBanner
                  banner={banner}
