...
const BookingSection = ({
  ...
  // Card selection state: when a calendar date has both regular + match cards
  const [cardSelectionData, setCardSelectionData] = useState(null);

  // ✅ State for stadium carousel (mobile only)
  // ✅ Fetch available tickets when date is selected (skip when booking a match card)
  useEffect(() => {
    ...
    // Skip regular ticket fetch — match card has its own tickets from parent
    if (matchCardId) {
      setAvailableTickets({ regularTickets: [], specialTickets: [] });
      return;
    }
  ...
  }, [selectedStadium, selectedDate, matchCardId]);
  ...
  useEffect(() => {
    ...
    if ((bookingStep === 'date' || bookingStep === 'payment') && selectedStadium) {
      // Always fetch when stadium changes (even if returning to same stadium)
      // This ensures we always get fresh data and handle cache issues
      const stadiumChanged = lastFetchedStadiumRef.current !== selectedStadium;
      
      if (stadiumChanged) {
        // Clear old videos immediately when stadium changes
        console.log('[BookingSection] Stadium changed from', lastFetchedStadiumRef.current, 'to', selectedStadium, '- clearing old videos and fetching new');
        setBookingVideos([]);
        ...
        const fetchVideos = async () => {
          ...
          try {
            ...
            console.log('[BookingSection] Videos response for stadium', selectedStadium, ':', response.data);
            ...
            if (response.data && Array.isArray(response.data)) {
              if (response.data.length > 0) {
                ...
                if (validVideos.length > 0) {
                  ...
                  console.log('[BookingSection] ⚠️ No valid videos found after filtering for stadium', selectedStadium);
                  setBookingVideos([]);
                  lastFetchedStadiumRef.current = selectedStadium;
                }
              } else {
                console.log('[BookingSection] ℹ️ No videos found for stadium', selectedStadium, '- backend returned empty array (no videos configured)');
                setBookingVideos([]);
                lastFetchedStadiumRef.current = selectedStadium;
              }
            } else {
        ...
        // Same stadium, but check if we have videos
        if (bookingVideos.length === 0) {
          // If we don't have videos but stadium is same, might be a cache issue - refetch
          console.log('[BookingSection] Same stadium', selectedStadium, 'but no videos - refetching to ensure fresh data');
          const fetchVideos = async () => {
            setLoadingVideos(true);
            try {
              const timestamp = Date.now();
              ...
              if (response.data && Array.isArray(response.data) && response.data.length > 0) {
                const validVideos = response.data.filter(video => {
                  return video.video_url && typeof video.video_url === 'string' && video.video_url.trim() !== '';
                });
                if (validVideos.length > 0) {
                  setBookingVideos(validVideos);
                  console.log('[BookingSection] ✅ Refetched and set', validVideos.length, 'videos for stadium', selectedStadium);
                }
              }
            } catch (error) {
              console.error('[BookingSection] Error refetching videos:', error);
            } finally {
              setLoadingVideos(false);
            }
          };
      ...
      <section id="booking" className={`py-12 sm:py-16 px-4 sm:px-6 lg:px-8 ${sectionBgClass}`} style={sectionBgStyle}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-16">
            <Skeleton className="h-8 w-64 mx-auto rounded" variant="dark" />
            <Skeleton className="h-4 w-48 mx-auto mt-4 rounded" variant="dark" />
          </div>
        </div>
      </section>
    );
  ...
  // Don't render section when on stadium step (no content to show)
  ...
  return (
    <section id="booking" className={`py-12 sm:py-16 px-4 sm:px-6 lg:px-8 ${sectionBgClass}`} style={sectionBgStyle}>
      <div className="max-w-7xl mx-auto">
                    ...
                    <AnimatedItem key={index} delay={index * 50}>
                      {(() => {
                        const matchCardsForDate = matchCards.filter(
                          mc => (mc.stadiumId || mc.stadium_id) === selectedStadium && mc.date === item.dateString
                        );
                        const hasAvailableMatchCard = matchCardsForDate.some(
                          mc => mc.tickets && mc.tickets.length > 0 && mc.tickets.some(t => t.quantity > 0)
                        );
                        // Date is interactable if regular tickets available OR a match card has tickets
                      ...
                      <div
                        onClick={() => {
                          ...
                          if (matchCardsForDate.length > 0) {
                            ...
                            navigate(getBookingPageUrl(selectedStadium, { date: item.dateString, step: 'payment' }), { replace: true });
                      ...
                      >
                        <div className="text-center">
                          <div className={`text-xs sm:text-sm uppercase mb-1 ${
                            !isInteractable ? 'text-red-400' : 'text-gray-400'
                          }`}>
                            {dayName}
                          </div>
        ...
        {/* Card Selection Modal — shown when a calendar date has both regular + match cards */}
        {cardSelectionData && (() => {
          ...
          const regularImage = (() => {
            ...
            const special = specialMatches?.find(m => (m.stadiumId || m.stadium_id) === selectedStadium && m.date === cardSelectionData.dateString);
            if (special?.image) return special.image;
            // 2. Daily image (exact date)
            const daily = dailyImages?.find(img => (img.stadiumId || img.stadium_id) === selectedStadium && img.date === cardSelectionData.dateString);
            if (daily?.image) return daily.image;
            // 3. Scheduled image — schedule.days is an array of day-of-week numbers
            const schedules = stadiumImageSchedules?.[selectedStadium] || [];
            ...
            return defaultStadiumImages[selectedStadium] || `/images/stadiums/${selectedStadium}.webp`;
          })();
          const regularName = getMatchName(selectedStadium, dow, cardSelectionData.dateString, stadiumImageSchedules, specialMatches, dailyImages) || (language === 'th' ? 'แมตช์ปกติ' : 'Regular Match');
          const allSelections = [
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Cards — scrollable horizontally on very small screens, grid on larger */}
                    ...
                    {allSelections.map((sel, idx) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (sel.type === 'regular') {
                            navigate(getBookingPageUrl(selectedStadium, { date: cardSelectionData.dateString, step: 'payment' }), { replace: true });
                          } else {
                            navigate(getBookingPageUrl(selectedStadium, { date: cardSelectionData.dateString, step: 'payment', matchCardId: sel.id }), { replace: true });
                          }
                          window.scrollTo({ top: 0, behavior: 'smooth' });
                        }}
                      ...
                      >
                        {/* Image fills a tall aspect — matches real event cards */}
                        <div className="relative w-full overflow-hidden rounded-xl"
                          ...
                          <img
                            ...
                            onError={e => { e.target.src = `/images/stadiums/${selectedStadium}.webp`; e.target.onerror = null; }}
                          />
                          {/* Sold Out overlay */}
                          {sel.isSoldOut && (
        ...
        {bookingStep === 'payment' && selectedStadium && selectedDate && (() => {
          ...
          // ✅ If matchCardId is set but tickets haven't loaded yet — show spinner
          if (matchCardId && matchCardTickets.length === 0) {
            return (
          ...
          // ✅ If matchCardId is set, show ONLY match card tickets (not regular/special)
          let allTickets;
          if (matchCardId && matchCardTickets.length > 0) {
            allTickets = matchCardTickets.map(ticket => ({
              ...ticket,
              type: 'matchcard',
              ticketId: String(ticket.id),
              ticketType: 'matchcard',
              id: String(ticket.id),
              name: ticket.name,
              price: ticket.price,
            ...
            // ✅ Regular flow: use available tickets from state (fetched from backend API)
          ...
          return (
            <div>
              <div className="flex items-center justify-between mb-8">
                <button
                  onClick={(e) => {
                    ...
                    if (selectedStadium && selectedDate) {
                      navigate(getBookingPageUrl(selectedStadium, { date: selectedDate, step: 'date' }), { replace: true });
                      setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 100);
                    } else if (selectedStadium) {
                      navigate(getBookingPageUrl(selectedStadium, { step: 'date' }), { replace: true });
                      setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 100);
                    } else {
              ...
              {/* Promotional / Seating Image Section
                  — when booking from a match card and seatingImage is set, show it instead */}
              {(matchCardSeatingImage || getPaymentImageForDate || selectedStadiumData?.paymentImage) && (
                <div className="mb-6 flex justify-center">
                  <div className="w-full max-w-2xl">
                    <img
                      src={matchCardSeatingImage || getPaymentImageForDate || selectedStadiumData.paymentImage}
                      alt={matchCardSeatingImage ? 'Seating layout' : 'Muay Thai stadium promotional image'}
                      width={672}
                      height={336}
              ...
              {/* Ticket Selection - ซ่อนเมื่อมี selectedZone แล้ว */}
              {!selectedZone && (
                <div className="mb-6">
                    ...
                    {zones.map(zone => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          navigate(getBookingPageUrl(selectedStadium, { date: selectedDate, zone: zone.id, step: 'payment' }), { replace: true });
                      ...
                      return (
                        <div className="relative">
                          ...
                          <div
                            ...
                            onClick={() => {
                              if (!isOutOfStock) {
                                const ticketZone = `${ticket.type}-${ticket.ticketId}`;
                                setSelectedZone(ticketZone);
                                
                                navigate(getBookingPageUrl(selectedStadium, { date: selectedDate, ticket: ticketZone, step: 'payment' }), { replace: true });
                                window.scrollTo({ top: 0, behavior: 'instant' });
                              }
                            }}
              ...
              {/* Ticket Type Detail Section - ซ่อนเมื่อมี selectedZone แล้ว */}
              {!selectedZone && (
              <div className="mb-6 bg-gray-900 rounded-lg p-4 sm:p-6 border border-gray-700">
              ...
              {/* Quantity and Total - แสดงเมื่อมี selectedZone */}
              {selectedZone && (() => {
