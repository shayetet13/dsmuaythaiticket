...
const UpcomingFightsSection = ({ 
  ...
  const checkAvailability = async (stadiumId, dateString, forceRefresh = false) => {
    ...
    try {
      // Use timestamp in params for cache busting instead of headers (to avoid CORS issues)
      const params = forceRefresh 
        ? { stadiumId, date: dateString, _t: Date.now() }
        : { stadiumId, date: dateString };
  ...
  useEffect(() => {
    const fetchAllAvailability = async () => {
      ...
      // Also fetch availability for match card dates
      if (matchCards && matchCards.length > 0) {
        matchCards.forEach(card => {
          if (card.date && card.stadiumId) {
            promises.push(checkAvailability(card.stadiumId, card.date, true));
          }
        });
      }
  ...
  }, [stadiums, specialMatches, dailyImages, matchCards]);
  ...
  const getStadiumEvents = (stadiumId) => {
    ...
    for (let i = 0; i <= 56; i++) { // Start from today, check next 8 weeks (for stadiums with few fight days)
      // Create date using local date components to avoid timezone issues
      const checkDate = new Date(todayYear, todayMonth, todayDay + i, 12, 0, 0);
      const dayOfWeek = checkDate.getDay();
      
      if (scheduleDays.includes(dayOfWeek) && upcomingDates.length < eventLimit) {
        const year = checkDate.getFullYear();
        const month = String(checkDate.getMonth() + 1).padStart(2, '0');
        const day = String(checkDate.getDate()).padStart(2, '0');
        const dateStr = `${year}-${month}-${day}`;
    ...
    // Add match cards as ADDITIONAL cards (never replace existing regular cards)
    if (matchCards && matchCards.length > 0) {
      matchCards
        .filter(card => card.stadiumId === stadiumId && card.date >= todayDateStr)
        .forEach(card => {
          try {
            const cacheKey = `${stadiumId}-${card.date}`;
            const hasNoTickets = !card.tickets || card.tickets.length === 0;
            const isSoldOut = hasNoTickets || (availabilityCache[cacheKey] !== undefined ? !availabilityCache[cacheKey] : false);
            sortedEvents.push({
              id: `matchcard-${card.id}`,
              name: card.name,
              date: card.date,
              image: card.image,
              price: null,
              ...
              isSpecialMatch: card.isSpecial || false
            });
          } catch (e) {
            console.error('Error processing match card:', e);
          }
        });
      // Sort: by date first, then by position within same date
      // showBefore=true → position 0 (before regular), regular → 1, showBefore=false → 2 (after regular)
      sortedEvents.sort((a, b) => {
        const dateA = new Date(a.date).getTime();
        const dateB = new Date(b.date).getTime();
        if (dateA !== dateB) return dateA - dateB;
        const posA = a.isMatchCard ? (a.showBefore ? 0 : 2) : 1;
        const posB = b.isMatchCard ? (b.showBefore ? 0 : 2) : 1;
        return posA - posB;
      });
    }
      ...
      specialMatches.forEach(match => {
        if (match.stadiumId === stadiumId && match.date) {
          try {
            ...
            if (daysDiff > 3 && !isAlreadyInRegularCards && sortedEvents.length < eventLimit) {
    ...
    // Cap at 8 cards (2 rows × 4 columns) to avoid layout issues
  ...
  // Re-compute events when availabilityCache or eventLimit changes
  const eventsByStadium = React.useMemo(() => {
    return getAllStadiumEvents();
  }, [stadiums, specialMatches, stadiumImageSchedules, matchCards, availabilityCache, eventLimit]);
  
  // Group stadiums by location for display (filter by singleStadiumId when set)
  const getStadiumsByLocation = () => {
    if (!stadiums || stadiums.length === 0) return { bangkok: [], phuket: [] };
    
    let filteredStadiums = stadiums;
    if (singleStadiumId) {
      const found = stadiums.find(s => s && s.id === singleStadiumId);
      filteredStadiums = found ? [found] : [];
    }
      ...
      <div className={`rounded-lg transition-all duration-300 relative ${
        ...
        >
          ...
          <div className="relative h-80 overflow-hidden bg-gray-200">
            <img
              ...
              alt={`${eventName} at ${stadiumName} on ${formatDate(event.date)} - Buy Muay Thai tickets online for ${stadiumName}`}
              width={400}
              height={320}
        ...
        <div className="p-4 bg-gray-50 border-t-2 border-gray-200">
          ...
          <button
            onClick={() => {
              if (!isSoldOut) {
                const navParams = { date: event.date, step: 'payment' };
                if (event.isMatchCard && event.matchCardId) navParams.matchCardId = String(event.matchCardId);
                navigate(getBookingPageUrl(event.stadiumId, navParams));
              }
            }}
            disabled={isSoldOut}
    ...
    <section id="tickets" className="relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 overflow-hidden bg-white">
      <div className="mx-auto max-w-7xl">
        <div className="text-center mb-8 sm:mb-12 md:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 mb-2 sm:mb-4 uppercase tracking-tight">
            ...
            {bangkokStadiums.map((stadium) => {
              ...
              return (
                <div key={stadium.id} className={bangkokStadiums.indexOf(stadium) < bangkokStadiums.length - 1 ? 'mb-12 sm:mb-16' : ''}>
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-red-600 mb-6 sm:mb-8 uppercase tracking-tight">
                  ...
                  {stadiumEvents.length > 0 ? (
                    <div className="relative px-8 sm:px-0">
                      ...
                      <div className="relative overflow-hidden rounded-lg w-full sm:overflow-visible">
                        ...
                        {/* Desktop: always 4-column grid (wraps to 2nd row for 5–8 cards) */}
                        <div className="hidden sm:grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
                          {stadiumEvents.map((event, index) => (
                            <div key={event.id || `${stadium.id}-${index}`}>
                              <EventCard event={event} />
                            </div>
                          ))}
                        </div>
