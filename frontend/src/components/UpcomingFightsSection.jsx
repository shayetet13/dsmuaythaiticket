...
const UpcomingFightsSection = ({ 
      ...
      specialMatches.forEach(match => {
        if (match.stadiumId === stadiumId && match.date) {
          try {
            ...
            // Check if this special match is already in the regular cards
            const isAlreadyInRegularCards = sortedEvents.some(event => event.date === match.date);
            
            // If special match is more than 3 days away and not already in regular cards, add as extra card
  ...
  }, [stadiums, specialMatches, stadiumImageSchedules, availabilityCache, eventLimit]);
      ...
      <div className={`rounded-lg transition-all duration-300 relative ${
        ...
        <div className="p-4 bg-gray-50 border-t-2 border-gray-200">
          ...
          <button
            onClick={() => {
              if (!isSoldOut) {
                navigate(getBookingPageUrl(event.stadiumId, { date: event.date, step: 'payment' }));
              }
            }}
            disabled={isSoldOut}
                        ...
                        <div className={`hidden sm:flex gap-4 sm:gap-6 ${
                          ...
                          {stadiumEvents.map((event, index) => (
                            <div 
                              ...
                              className={stadiumEvents.length <= 8
