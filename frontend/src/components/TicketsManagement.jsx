...
import { Ticket, Calendar, MapPin, Plus, X, Save, Trash2, Edit2, ArrowLeft, ChevronLeft, Upload, Image as ImageIcon } from 'lucide-react';
  ...
  const handleUpdateSpecialTicket = async (ticketId, name, price, date, quantity, image) => {
    ...
    try {
      ...
      if (image && typeof image === 'string' && image.startsWith('data:image')) {
        // If it's a new image (base64), compress it
        const response = await fetch(image);
        const blob = await response.blob();
        const file = new File([blob], 'image.jpg', { type: blob.type });
        processedImage = await compressImage(file, 1920, 1080, 0.85);
      }
