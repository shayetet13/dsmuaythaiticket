...
const HighlightsSection = ({ highlights, highlightIndex, setHighlightIndex, prevHighlight, nextHighlight, language, t }) => {
  // Preload visible and next highlight images - defer to avoid competing with LCP
  useEffect(() => {
    const doPreload = () => {
      const imagesToPreload = [];
      if (highlights[highlightIndex]?.image) imagesToPreload.push(highlights[highlightIndex].image);
      const nextIndex = (highlightIndex + 1) % highlights.length;
      if (highlights[nextIndex]?.image) imagesToPreload.push(highlights[nextIndex].image);
    ...
    if ('requestIdleCallback' in window) {
      ...
      const t = setTimeout(doPreload, 500);
  ...
  return (
    <section id="highlight" className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <AnimatedSection>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-4 text-red-600">{t.highlight.title}</h2>
                  ...
                  <div className="flex flex-col md:grid md:grid-cols-2 gap-0 bg-white rounded-lg overflow-hidden border-2 border-gray-300 w-full">
                    ...
                    <div className="relative h-48 sm:h-64 md:h-96 overflow-hidden w-full order-1 md:order-1">
                      <OptimizedImage
                        ...
                        src={highlight.image}
                        alt={`${highlight.title} - Muay Thai fight highlights at ${highlight.date || 'recent event'} - Book tickets for upcoming Muay Thai matches`}
                        width={800}
                        height={384}
                    ...
                    <div className="p-4 sm:p-6 md:p-8 lg:p-10 flex flex-col justify-center bg-gray-50 w-full overflow-hidden order-2 md:order-2 border-l-2 border-gray-300">
                      ...
                      <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-3 sm:mb-4 md:mb-6 leading-tight break-words">
                        {highlight.title}
                      </h3>
