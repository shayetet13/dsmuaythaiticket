...
const BookingDetail = ({ booking, onBack }) => {
  const ticketDisplay = React.useMemo(() => {
    if (booking.ticket_id && booking.ticket_type) {
      const typeLabel = booking.ticket_type === 'regular' ? 'ตั๋วปกติ' : 'ตั๋วพิเศษ';
      const name = booking.ticket_name || 'ไม่ระบุชื่อ';
      return { name, typeLabel };
    }
    if (booking.zone_name) return { name: booking.zone_name, typeLabel: null };
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-white font-medium mt-1">{ticketDisplay.name}</div>
