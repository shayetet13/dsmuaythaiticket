...
const ImagesManagement = () => {
  ...
  // Upcoming Fight Banner state
  const [bannerTargetId, setBannerTargetId] = useState('homepage');
  const [upcomingFightBanners, setUpcomingFightBanners] = useState([]);
  const [allMatchCards, setAllMatchCards] = useState([]);
  const [bannerSaving, setBannerSaving] = useState(false);
  // Per-tab form state — keyed by targetId so switching tabs never loses unsaved changes
  const [bannerFormsMap, setBannerFormsMap] = useState({});
  const [bannerPickMap, setBannerPickMap] = useState({});

  const defaultBannerForm = { matchCardId: null, countdownTo: '', isVisible: false, isAuto: false, eventName: '', eventDate: '', eventImage: '', eventStadiumId: '' };
  const defaultBannerPick = { stadiumId: '', date: '' };

  const currentBannerForm = bannerFormsMap[bannerTargetId] || defaultBannerForm;
  const currentBannerPick = bannerPickMap[bannerTargetId] || defaultBannerPick;

  const updateBannerForm = (targetId, updates) =>
    setBannerFormsMap(prev => ({ ...prev, [targetId]: { ...(prev[targetId] || defaultBannerForm), ...updates } }));

  const updateBannerPick = (targetId, updates) =>
    setBannerPickMap(prev => ({ ...prev, [targetId]: { ...(prev[targetId] || defaultBannerPick), ...updates } }));
  ...
  useEffect(() => {
    ...
    if (activeSection === 'upcoming-fight-banner') {
      loadUpcomingFightBannerData();
    }
  }, [activeSection]);
  ...
  const loadUpcomingFightBannerData = async () => {
    try {
      const [banners, cards] = await Promise.all([
        getUpcomingFightBanners(),
        getMatchCardsApi()
      ]);
      setUpcomingFightBanners(banners || []);
      setAllMatchCards(cards || []);
      // Initialise per-tab form map from saved banners — skip tabs already edited by user
      setBannerFormsMap(prev => {
        const next = { ...prev };
        (banners || []).forEach(b => {
          if (!next[b.targetId]) {
            next[b.targetId] = {
              matchCardId: b.matchCardId || null,
              countdownTo: b.countdownTo ? b.countdownTo.slice(0, 16) : '',
              isVisible: b.isVisible !== false,
              isAuto: !!b.isAuto,
              eventName: b.eventName || '',
              eventDate: b.eventDate || '',
              eventImage: b.eventImage || '',
              eventStadiumId: b.eventStadiumId || ''
            };
          }
        });
        return next;
      });
      setBannerPickMap(prev => {
        const next = { ...prev };
        (banners || []).forEach(b => {
          if (!next[b.targetId]) {
            next[b.targetId] = { stadiumId: b.eventStadiumId || '', date: b.eventDate || '' };
          }
        });
        return next;
      });
    } catch (error) {
      console.error('Error loading upcoming fight banner data:', error);
    }
  };
  ...
  const sections = [
    ...
    { id: 'upcoming-fight-banner', label: 'Up coming fight' },
  ...
  // Upcoming Fight Banner Management
  const handleBannerSave = async () => {
    const form = currentBannerForm;
    // Auto mode requires only visibility; manual mode requires an event selection
    if (!form.isAuto && (!form.eventName || !form.eventDate || !form.eventStadiumId)) {
      showMessage('error', 'กรุณาเลือกกิจกรรมก่อนบันทึก');
      return;
    }
    setBannerSaving(true);
    try {
      const result = await upsertUpcomingFightBanner(bannerTargetId, {
        matchCardId: form.isAuto ? null : (form.matchCardId || null),
        countdownTo: form.isAuto ? null : (form.countdownTo || null),
        isVisible: form.isVisible,
        isAuto: !!form.isAuto,
        eventName: form.isAuto ? null : (form.eventName || null),
        eventDate: form.isAuto ? null : (form.eventDate || null),
        eventImage: form.isAuto ? null : (form.eventImage || null),
        eventStadiumId: form.isAuto ? bannerTargetId : (form.eventStadiumId || null)
      });
      if (result) {
        showMessage('success', 'บันทึก Up Coming Fight Banner สำเร็จ');
        // Clear this tab's cache so it re-reads fresh data
        setBannerFormsMap(prev => { const n = { ...prev }; delete n[bannerTargetId]; return n; });
        setBannerPickMap(prev => { const n = { ...prev }; delete n[bannerTargetId]; return n; });
        await loadUpcomingFightBannerData();
      } else {
  ...
  const handleBannerClear = async (targetId) => {
    if (!window.confirm(`ยืนยันการล้างการตั้งค่า Up Coming Fight สำหรับ "${targetId === 'homepage' ? 'Homepage' : targetId}"?`)) return;
    try {
      const ok = await clearUpcomingFightBanner(targetId);
      if (ok) {
        showMessage('success', 'ล้างการตั้งค่า Banner สำเร็จ');
        // Clear this tab's local form cache so it resets to default
        setBannerFormsMap(prev => { const n = { ...prev }; delete n[targetId]; return n; });
        setBannerPickMap(prev => { const n = { ...prev }; delete n[targetId]; return n; });
        await loadUpcomingFightBannerData();
      } else {
        showMessage('error', 'เกิดข้อผิดพลาดในการล้างค่า');
      }
    } catch (err) {
  ...
  return (
    <div>
      ...
      {/* Upcoming Fight Banner Management Section */}
      {activeSection === 'upcoming-fight-banner' && (
        <div>
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white mb-2">Up Coming Fight Banner</h2>
            <p className="text-gray-400 text-sm">
              จัดการแบนเนอร์ Up Coming Fight ที่จะแสดงบนหน้าแรกและหน้าสนาม — เลือก Match Card, ตั้งเวลานับถอยหลัง และเปิด/ปิดการแสดงผล
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button
              onClick={() => setBannerTargetId('homepage')}
              className={`px-4 py-2 rounded text-sm font-semibold transition-colors ${bannerTargetId === 'homepage' ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              Homepage
            </button>
            {(data?.stadiums || []).map(stadium => (
              <button
                key={stadium.id}
                onClick={() => setBannerTargetId(stadium.id)}
                className={`px-4 py-2 rounded text-sm font-semibold transition-colors ${bannerTargetId === stadium.id ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
              >
                {typeof stadium.name === 'string' ? stadium.name : stadium.name?.en || stadium.name?.th || stadium.id}
              </button>
            ))}
          </div>
          ...
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 max-w-2xl">
            <h3 className="text-lg font-semibold text-white mb-5 capitalize">
              ตั้งค่าสำหรับ: <span className="text-yellow-400">{bannerTargetId === 'homepage' ? 'Homepage' : bannerTargetId}</span>
            </h3>

            {/* Visibility Toggle */}
            <div className="flex items-center gap-3 mb-4">
              <button
                onClick={() => updateBannerForm(bannerTargetId, { isVisible: !currentBannerForm.isVisible })}
                className={`relative w-12 h-6 rounded-full transition-colors duration-200 focus:outline-none ${currentBannerForm.isVisible ? 'bg-green-500' : 'bg-gray-600'}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${currentBannerForm.isVisible ? 'translate-x-6' : ''}`} />
              </button>
              <span className="text-gray-300 text-sm">
                {currentBannerForm.isVisible ? 'แสดงบนเว็บไซต์' : 'ซ่อน (ไม่แสดงบนเว็บไซต์)'}
              </span>
            </div>

            {/* Auto Toggle — stadium pages only (not homepage) */}
            {bannerTargetId !== 'homepage' && (
              <div className="mb-5 p-4 rounded-lg border border-blue-500/30 bg-blue-500/5">
                <div className="flex items-center gap-3 mb-2">
                  <button
                    onClick={() => updateBannerForm(bannerTargetId, { isAuto: !currentBannerForm.isAuto })}
                    className={`relative w-12 h-6 rounded-full transition-colors duration-200 focus:outline-none ${currentBannerForm.isAuto ? 'bg-blue-500' : 'bg-gray-600'}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${currentBannerForm.isAuto ? 'translate-x-6' : ''}`} />
                  </button>
                  <span className="text-blue-300 text-sm font-semibold">Auto — ดึงกิจกรรมอัตโนมัติ</span>
                  {currentBannerForm.isAuto && (
                    <span className="ml-auto text-xs px-2 py-0.5 rounded bg-blue-500/20 text-blue-400 font-semibold">เปิดใช้งาน</span>
                  )}
                </div>
                {currentBannerForm.isAuto && (
                  <div className="text-gray-400 text-xs space-y-1 mt-2 pl-1">
                    <p>• ดึงภาพจาก <span className="text-white">แก้ไขภาพสนาม</span> เท่านั้น (ตารางประจำสัปดาห์)</p>
                    <p>• นับถอยหลังถึง <span className="text-white">20:30 น. เวลาไทย</span> ของวันที่มีการแข่งขัน</p>
                    <p>• หลังเที่ยงคืน (00:00 น. เวลาไทย) จะเปลี่ยนการ์ดไปวันถัดไปที่มีตารางอัตโนมัติ</p>
                    <p>• เมื่อเปิด Auto: การตั้งกิจกรรมด้วยตนเองด้านล่างจะไม่ถูกใช้งาน</p>
                  </div>
                )}
              </div>
            )}

            {/* Event Selector — 3-step drill-down: Stadium → Date → Event (hidden in Auto mode) */}
            {currentBannerForm.isAuto ? (
              <div className="mb-5 p-4 rounded-lg border border-dashed border-gray-600 text-center">
                <p className="text-gray-500 text-sm">การเลือกกิจกรรมด้วยตนเองถูกปิดใช้งาน (Auto Mode เปิดอยู่)</p>
              </div>
            ) : null}
            {!currentBannerForm.isAuto && (() => {
              // Helper: compute next N occurrences of specific weekdays from today
              const getUpcomingDatesForDays = (dayNumbers, weeksAhead = 8) => {
              ...
              const pickStadiumId = currentBannerPick.stadiumId;
              const pickDate = currentBannerPick.date;

              // Build all available dates for selected stadium from all sources
              const scheduleDates = (() => {
                if (!pickStadiumId || !data?.stadiumImageSchedules) return [];
                const schedules = data.stadiumImageSchedules[pickStadiumId] || [];
                const allDays = [...new Set(schedules.flatMap(s => s.days || []))];
                if (allDays.length === 0) return [];
                return getUpcomingDatesForDays(allDays);
              })();

              const specialDates = (() => {
                if (!pickStadiumId || !data?.specialMatches) return [];
                return (data.specialMatches || [])
                  .filter(m => m.stadiumId === pickStadiumId)
                  .map(m => m.date);
              })();

              const dailyImageDates = (() => {
                if (!pickStadiumId || !data?.dailyImages) return [];
                return (data.dailyImages || [])
                  .filter(img => img.stadiumId === pickStadiumId)
                  .map(img => img.date);
              })();

              const matchCardDates = (() => {
                if (!pickStadiumId) return [];
                return allMatchCards
                  .filter(c => c.stadiumId === pickStadiumId)
                  .map(c => c.date);
              })();

              const allDates = [...new Set([...scheduleDates, ...dailyImageDates, ...specialDates, ...matchCardDates])].sort();

              // Build events for selected date
              const eventsForDate = (() => {
                if (!pickStadiumId || !pickDate) return [];
                const events = [];

                // Schedule (regular weekday) events
                if (data?.stadiumImageSchedules) {
                  const schedules = data.stadiumImageSchedules[pickStadiumId] || [];
                  const dayOfWeek = new Date(pickDate + 'T00:00:00').getDay();
                  schedules.forEach(s => {
                    if ((s.days || []).includes(dayOfWeek)) {
                      events.push({
                        key: `schedule-${s.id}`,
                        label: s.name || `ตารางประจำวัน (${pickDate})`,
                        name: s.name || `Muay Thai – ${pickDate}`,
                        date: pickDate,
                        image: s.image || '',
                        stadiumId: pickStadiumId,
                        matchCardId: null,
                        sourceType: 'schedule'
                      });
                    }
                ...
                // Daily images on this date (แก้ไขภาพประจำวัน)
                if (data?.dailyImages) {
                  (data.dailyImages || [])
                    .filter(img => img.stadiumId === pickStadiumId && img.date === pickDate)
                    .forEach(img => {
                      events.push({
                        key: `daily-${img.id}`,
                        label: img.name || `ภาพประจำวัน – ${img.date}`,
                        name: img.name || `Daily Match – ${img.date}`,
                        date: img.date,
                        image: img.image || '',
                        stadiumId: pickStadiumId,
                        matchCardId: null,
                ...
                // Special matches on this date
                if (data?.specialMatches) {
                  (data.specialMatches || [])
                    .filter(m => m.stadiumId === pickStadiumId && m.date === pickDate)
                    .forEach(m => {
                      events.push({
                        key: `special-${m.id}`,
                        label: m.name || `แมตช์พิเศษ – ${m.date}`,
                        name: m.name || `Special Match – ${m.date}`,
                        date: m.date,
                        image: m.image || '',
                        stadiumId: pickStadiumId,
                        matchCardId: null,
                ...
                // Match cards on this date
                allMatchCards
                  .filter(c => c.stadiumId === pickStadiumId && c.date === pickDate)
                  .forEach(c => {
                    events.push({
                      key: `matchcard-${c.id}`,
                      label: c.name || `Match Card – ${c.date}`,
                      name: c.name,
                      date: c.date,
                      image: c.image || '',
                      stadiumId: pickStadiumId,
                      matchCardId: c.id,
                      sourceType: 'matchcard'
                    });
                  });
              ...
              const sourceTypeBadge = { schedule: 'ตารางประจำ', daily: 'ภาพประจำวัน', special: 'แมตช์พิเศษ', matchcard: 'การ์ดแมตช์' };

              return (
                <div className="mb-5 space-y-3">
                  <label className="block text-sm font-medium text-gray-300">เลือกกิจกรรม *</label>

                  {/* Step 1: Stadium */}
                  <div>
                    <p className="text-xs text-gray-500 mb-1">1. เลือกสนาม</p>
                    <select
                      value={pickStadiumId}
                      onChange={e => {
                        updateBannerPick(bannerTargetId, { stadiumId: e.target.value, date: '' });
                        updateBannerForm(bannerTargetId, { matchCardId: null, eventName: '', eventDate: '', eventImage: '', eventStadiumId: '', countdownTo: '' });
                      }}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-yellow-500"
                    >
                      <option value="">-- เลือกสนาม --</option>
                      {(data?.stadiums || []).map(s => (
                        <option key={s.id} value={s.id}>
                          {typeof s.name === 'string' ? s.name : s.name?.en || s.name?.th || s.id}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  {pickStadiumId && (
                    <div>
                      <p className="text-xs text-gray-500 mb-1">2. เลือกวันที่ ({allDates.length} วัน)</p>
                      {allDates.length === 0 ? (
                        <p className="text-gray-500 text-sm italic">ไม่มีกิจกรรมที่กำหนดไว้สำหรับสนามนี้</p>
                      ) : (
                        <select
                          value={pickDate}
                          onChange={e => {
                            updateBannerPick(bannerTargetId, { date: e.target.value });
                            updateBannerForm(bannerTargetId, { matchCardId: null, eventName: '', eventDate: '', eventImage: '', eventStadiumId: '', countdownTo: '' });
                          }}
                          className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-yellow-500"
                        >
                  ...
                  {/* Step 3: Event */}
                  {pickStadiumId && pickDate && (
                    <div>
                      <p className="text-xs text-gray-500 mb-1">3. เลือกกิจกรรม ({eventsForDate.length} รายการ)</p>
                      {eventsForDate.length === 0 ? (
                        <p className="text-gray-500 text-sm italic">ไม่มีกิจกรรมในวันที่เลือก</p>
                      ) : (
                        <div className="space-y-2">
                          {eventsForDate.map(ev => {
                            const isSelected = currentBannerForm.eventName === ev.name && currentBannerForm.eventDate === ev.date && currentBannerForm.eventStadiumId === ev.stadiumId;
                            return (
                              <button
                                key={ev.key}
                                type="button"
                                onClick={() => updateBannerForm(bannerTargetId, {
                                  matchCardId: ev.matchCardId || null,
                                  eventName: ev.name,
                                  eventDate: ev.date,
                                  eventImage: ev.image,
                                  eventStadiumId: ev.stadiumId,
                                  countdownTo: ev.date + 'T18:30'
                                })}
                                className={`w-full text-left p-3 rounded-lg border transition-colors flex items-center gap-3 ${isSelected ? 'border-yellow-500 bg-yellow-500/10' : 'border-gray-600 bg-gray-700/50 hover:border-gray-500'}`}
                              >
                                {ev.image && (
                                  <img src={ev.image} alt={ev.name} className="w-14 h-14 object-cover rounded flex-shrink-0" />
                                )}
                                <div className="flex-1 min-w-0">
                                  <p className="text-white text-sm font-semibold truncate">{ev.name}</p>
                                  <p className="text-gray-400 text-xs mt-0.5">{ev.date}</p>
                                  <span className={`inline-block text-xs px-2 py-0.5 rounded mt-1 ${ev.sourceType === 'matchcard' ? 'bg-blue-500/20 text-blue-400' : ev.sourceType === 'special' ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20 text-gray-400'}`}>
                                    {sourceTypeBadge[ev.sourceType]}
                                  </span>
                                </div>
                                {isSelected && <span className="text-yellow-500 text-lg">✓</span>}
                              </button>
                            );
                          })}
                        </div>
            ...
            {/* Selected Event Preview (hidden in Auto mode) */}
            {!currentBannerForm.isAuto && currentBannerForm.eventName && (
              <div className="mb-5 p-4 bg-yellow-500/5 rounded-lg border border-yellow-500/30 flex gap-4 items-start">
                {currentBannerForm.eventImage && (
                  <img src={currentBannerForm.eventImage} alt={currentBannerForm.eventName} className="w-20 h-20 object-cover rounded flex-shrink-0" />
                )}
                <div>
                  <p className="text-yellow-400 text-xs font-semibold uppercase mb-1">กิจกรรมที่เลือก</p>
                  <p className="text-white font-semibold text-sm">{currentBannerForm.eventName}</p>
                  <p className="text-gray-400 text-xs mt-1">วันที่: {currentBannerForm.eventDate}</p>
                  <p className="text-gray-400 text-xs">สนาม: {currentBannerForm.eventStadiumId}</p>
                </div>
              </div>
            )}

            {/* Countdown Datetime (hidden in Auto mode — auto always uses 20:30 Thailand time) */}
            {!currentBannerForm.isAuto && <div className="mb-5">
              <label className="block text-sm font-medium text-gray-300 mb-2">
                เวลานับถอยหลังถึง (Countdown Target)
              </label>
              <input
                type="datetime-local"
                value={currentBannerForm.countdownTo}
                onChange={e => updateBannerForm(bannerTargetId, { countdownTo: e.target.value })}
                className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-yellow-500"
              />
              <p className="text-gray-500 text-xs mt-1">หากไม่กำหนด จะใช้วันที่ของกิจกรรมเป็นเวลานับถอยหลัง</p>
            </div>}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2 bg-yellow-500 text-black font-semibold rounded hover:bg-yellow-400 transition-colors disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
                {bannerSaving ? 'กำลังบันทึก...' : 'บันทึก'}
              </button>
              {upcomingFightBanners.some(b => b.targetId === bannerTargetId) && (
                <button
                  onClick={() => handleBannerClear(bannerTargetId)}
                  className="flex items-center gap-2 px-6 py-2 bg-gray-700 text-red-400 font-semibold rounded hover:bg-red-600 hover:text-white border border-red-600/40 transition-colors"
                >
          ...
          {/* Summary Table */}
          {upcomingFightBanners.length > 0 && (
            <div className="mt-8">
              <h3 className="text-lg font-semibold text-white mb-3">สรุปการตั้งค่าทั้งหมด</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-gray-300 border border-gray-700 rounded-lg overflow-hidden">
                  <thead>
                    <tr className="bg-gray-700 text-gray-200">
                      <th className="px-4 py-2 text-left">เป้าหมาย</th>
                      <th className="px-4 py-2 text-left">Match Card</th>
                      <th className="px-4 py-2 text-left">Countdown To</th>
                      <th className="px-4 py-2 text-left">สถานะ</th>
                    </tr>
                  </thead>
                  <tbody>
                    {upcomingFightBanners.map(b => (
                      <tr key={b.id} className="border-t border-gray-700 hover:bg-gray-700/30">
                        <td className="px-4 py-2 font-medium text-white">{b.targetId === 'homepage' ? 'Homepage' : b.targetId}</td>
                        <td className="px-4 py-2 max-w-xs truncate">
                          {b.isAuto
                            ? <span className="text-blue-400 text-xs font-semibold">Auto (ตารางประจำ 20:30)</span>
                            : b.eventName ? `[${b.eventDate}] ${b.eventName}` : '-'}
                        </td>
                        <td className="px-4 py-2">{b.isAuto ? '20:30 น. ไทย (อัตโนมัติ)' : (b.countdownTo ? b.countdownTo.replace('T', ' ') : '-')}</td>
                        <td className="px-4 py-2">
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${b.isVisible ? 'bg-green-500/20 text-green-400' : 'bg-gray-600/40 text-gray-400'}`}>
