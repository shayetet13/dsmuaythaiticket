...
import StadiumButtonsSection from './StadiumButtonsSection';

// Constants & Utils
import { translations } from '../constants/translations';
import { getZones } from '../constants/zones';
import { API_URL } from '../config/api.js';
import { HOME_PAGE_TITLE, DOCUMENT_TITLE_BRAND_SUFFIX } from '../config/constants.js';

// Hooks
import { useDatabase } from '../hooks/useDatabase';

const BookingPage = ({ stadiumIdFromPath: stadiumIdFromProps }) => {
  const navigate = useNavigate();
  const { stadiumId: stadiumIdFromParams } = useParams();
  const stadiumIdFromPath = stadiumIdFromProps ?? stadiumIdFromParams;
  const [searchParams] = useSearchParams();
  
  // Get initial values: stadium from path (e.g. /rajadamnern-stadium-ticket) or query params
  const initialStadium = stadiumIdFromPath || searchParams.get('stadium') || '';
  const initialDate = searchParams.get('date') || '';
  const initialZone = searchParams.get('ticket') || searchParams.get('zone') || '';
  const stepFromUrl = searchParams.get('step') || 'stadium';
  const initialMatchCardId = searchParams.get('matchCardId') || null;
  ...
  // Language state - sessionStorage: persists during session, resets to 'en' when user leaves and returns
  ...
  const [matchCardId, setMatchCardId] = useState(initialMatchCardId);
  const [matchCardTickets, setMatchCardTickets] = useState([]);
  const [matchCardSeatingImage, setMatchCardSeatingImage] = useState(null);
  
  // Ref to track if we're updating URL from state (to prevent infinite loop)
  const isUpdatingUrlFromState = useRef(false);

  // Load data from database
  const { stadiums, stadiumImageSchedules, specialMatches, dailyImages, bookingBackground, dbLoaded, matchCards } = useDatabase(language);
  const stadiumData = stadiums.find(s => s && s.id === selectedStadium);

  // Set document title for booking page
  React.useEffect(() => {
    if (stadiumData?.name) {
      ...
      document.title = `${baseName} Fight Schedule - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
    } else {
      document.title = HOME_PAGE_TITLE;
    }
    return () => { document.title = HOME_PAGE_TITLE; };
  }, [stadiumData]);
  ...
  const totalPrice = useMemo(() => {
    ...
    if (selectedZone.includes('-')) {
      ...
      if (type === 'matchcard') {
        // Match card tickets don't use ticketConfigs — look up from matchCardTickets directly
        ticket = matchCardTickets.find(t => String(t.id) === ticketId);
      } else {
      ...
      if (ticket && ticket.price !== undefined) {
    ...
    return 0;
  }, [selectedStadium, selectedZone, bookingForm.quantity, ticketConfigs, matchCardTickets, zones]);
  ...
  useEffect(() => {
    ...
    const stepFromUrl = searchParams.get('step') || 'stadium';
    const stadiumFromUrl = stadiumIdFromPath || searchParams.get('stadium') || '';
    const dateFromUrl = searchParams.get('date') || '';
    const zoneFromUrl = searchParams.get('ticket') || searchParams.get('zone') || '';
    ...
    // Sync zone/ticket from URL (when on payment step)
    if (stepFromUrl === 'payment' && zoneFromUrl && zoneFromUrl !== selectedZone) {
      setSelectedZone(zoneFromUrl);
    } else if (stepFromUrl === 'date' && selectedZone) {
      // Clear selectedZone when going back to date selection (but not when on payment step)
      setSelectedZone('');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams.toString(), stadiumIdFromPath]);
  ...
  useEffect(() => {
    ...
    if (bookingStep === 'stadium') {
      if (!selectedStadium && !dateFromUrl) {
        return;
      }
      // When on stadium step with a stadium, use fight-schedule URL
      const newUrl = selectedStadium
        ? getBookingPageUrl(selectedStadium, { step: 'stadium' })
        : '/';
      ...
      // Build expected URL: /{stadiumId}-fight-schedule?date=xxx&step=xxx
      const expectedUrl = selectedStadium
        ? getBookingPageUrl(selectedStadium, {
            date: selectedDate || undefined,
            step: bookingStep,
            ticket: selectedZone?.includes('-') ? selectedZone : undefined,
            zone: selectedZone && !selectedZone.includes('-') ? selectedZone : undefined,
            matchCardId: matchCardId || undefined
          })
        : '/';
      
      // Update URL when it doesn't match expected state
      if (expectedUrl !== currentUrl && expectedUrl !== '/') {
        isUpdatingUrlFromState.current = true;
        navigate(expectedUrl, { replace: true });
      }
    }
  }, [selectedStadium, selectedDate, selectedZone, bookingStep, navigate, matchCardId]);
  ...
  const handleBooking = async (e) => {
    ...
    if (selectedZone.includes('-')) {
      ...
      if (type === 'matchcard') {
        selectedTicketData = matchCardTickets.find(t => String(t.id) === id) || null;
      } else {
        const ticketConfig = ticketConfigs[selectedStadium];
        if (ticketConfig) {
          if (type === 'regular') {
  ...
  // Load match card data (tickets + seatingImage) when matchCardId is set
  useEffect(() => {
    if (!matchCardId) {
      setMatchCardTickets([]);
      setMatchCardSeatingImage(null);
      return;
    }
    axios.get(`${API_URL}/match-cards/${matchCardId}`)
      .then(res => {
        const card = res.data;
        setMatchCardTickets(Array.isArray(card.tickets) ? card.tickets : []);
        setMatchCardSeatingImage(card.seatingImage || null);
      })
      .catch(() => {
        setMatchCardTickets([]);
        setMatchCardSeatingImage(null);
  ...
  // Sync matchCardId from URL when URL changes
  useEffect(() => {
    const mcId = searchParams.get('matchCardId') || null;
    setMatchCardId(mcId);
  }, [searchParams]);
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      {/* Header - hide language toggle on payment page (English only for tourists) */}
      <Header
        language={bookingStep === 'payment' ? 'en' : language}
        toggleLanguage={toggleLanguage}
        t={bookingStep === 'payment' ? translations.en : t}
        mobileMenuOpen={mobileMenuOpen}
        setMobileMenuOpen={setMobileMenuOpen}
        navigate={navigate}
        hideLanguageToggle={bookingStep === 'payment'}
      />
      
      {/* Add padding top to account for fixed header */}
      <div className="pt-20">

      {/* Stadium buttons - only when selecting stadium (hide on date step - user came from upcoming fight) */}
      {dbLoaded && stadiums.length > 0 && bookingStep === 'stadium' && (
        <StadiumButtonsSection stadiums={stadiums} lightBackground={true} language={language} t={t} />
      )}
      ...
      bookingStep === 'email_verification' && paymentData ? (
        <EmailVerificationPage
          ...
          t={t}
        />
      ) : /* Payment QR Page - force English only for tourists */
      ...
      dbLoaded && stadiums.length > 0 ? (
        <BookingSection
          ...
          matchCardTickets={matchCardTickets}
          matchCardSeatingImage={matchCardSeatingImage}
