...
/**
 * Check if current time in Thailand has passed 17:00 (5:00 PM)
 * @returns {boolean} - True if current time is 17:00 or later Thailand time
 */
export const isAfter5PMThailand = () => {
  const currentThailandTime = getCurrentThailandTime();
  const hours = currentThailandTime.getHours();
  
  // Check if time is 17:00 or later (5:00 PM or later)
  return hours >= 17;
};

/**
 * Check if tickets can be purchased for a specific date
 * Tickets cannot be purchased for today's date after 17:00 Thailand time
 * @param {string} dateString - Date in YYYY-MM-DD format
 * @returns {boolean} - True if tickets can be purchased (false if past 17:00 for today)
 */
export const canPurchaseTicketsForDate = (dateString) => {
  // If it's today, check if current time is before 17:00
