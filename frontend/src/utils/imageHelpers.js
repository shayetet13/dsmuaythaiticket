...
/**
 * Compress image to exact size (fit - scale to fit within, add letterboxing)
 * For stadium highlight images: 1500x405 - ภาพทั้งหมดจะแสดงครบ ไม่ถูก crop
 * @param {File} file - Original image file
 ...
 * @returns {Promise<string>} - Base64 compressed image (WebP or JPEG)
 */
export const compressImageToExactSize = async (file, targetWidth = 1500, targetHeight = 405, quality = 0.85) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onload = async (event) => {
