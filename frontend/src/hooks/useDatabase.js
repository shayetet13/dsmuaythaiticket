...
import { initDb, getHighlights, getStadiums, getWeeklyFights, getHeroImage, getStadiumImageSchedules, getSpecialMatches, getDailyImages, getUpcomingFightsBackground, getBookingBackground, getStadiumHighlights, getStadiumHighlightImages, getStadiumDescriptions, getMatchCards, getUpcomingFightBanners } from '../db/imagesDb';
...
export const useDatabase = (language) => {
  ...
  const [stadiumHighlights, setStadiumHighlights] = useState({});
  const [stadiumHighlightImages, setStadiumHighlightImages] = useState({});
  const [stadiumDescriptions, setStadiumDescriptions] = useState({});
    ...
    const loadData = async () => {
      try {
        ...
        const [
          ...
          stadiumHighlightsData,
          stadiumHighlightImagesData,
          stadiumDescriptionsData,
          matchCardsData,
          upcomingFightBannersData
        ...
        if (stadiumHighlightsData.status === 'fulfilled') updateState(setStadiumHighlights, stadiumHighlightsData.value || {});
        if (stadiumHighlightImagesData.status === 'fulfilled') updateState(setStadiumHighlightImages, stadiumHighlightImagesData.value || {});
        if (stadiumDescriptionsData.status === 'fulfilled') updateState(setStadiumDescriptions, stadiumDescriptionsData.value || {});
        if (matchCardsData.status === 'fulfilled') updateState(setMatchCards, matchCardsData.value || []);
        if (upcomingFightBannersData.status === 'fulfilled') updateState(setUpcomingFightBanners, upcomingFightBannersData.value || []);
