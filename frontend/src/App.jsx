...
import StadiumButtonsSection from './components/StadiumButtonsSection';
import UpcomingFightBanner from './components/UpcomingFightBanner';
...
const App = () => {
  ...
  const { heroImage, highlights, stadiums, weeklyFights, stadiumImageSchedules, specialMatches, dailyImages, matchCards, upcomingFightBanners, dbLoaded } = useDatabase(language);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Update canonical URL (remove query params and hash for canonical)
      ...
      // Update meta description and title based on current page
      ...
      if (currentPath.includes('/booking') || currentPath.includes('/schedule/')) {
        title = language === 'th' 
          ? `จองตั๋วมวยไทยออนไลน์ - ${DOCUMENT_TITLE_BRAND_SUFFIX}`
          : `Book Muay Thai Tickets Online - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
        description = language === 'th'
          ? 'จองตั๋วมวยไทยออนไลน์สำหรับสนามมวยชั้นนำในกรุงเทพและภูเก็ต เลือกวันที่และที่นั่งที่ต้องการ ชำระเงินง่าย ปลอดภัย ได้ตั๋วทันที'
          : 'Book Muay Thai tickets online for top stadiums in Bangkok and Phuket. Select your preferred date and seats. Easy and secure payment with instant confirmation.';
      } else if (currentPath.includes('/payment') || bookingStep === 'payment') {
        title = language === 'th'
          ? `ชำระเงินตั๋วมวยไทย - ${DOCUMENT_TITLE_BRAND_SUFFIX}`
          : `Pay for Muay Thai Tickets - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
        description = language === 'th'
          ? 'ชำระเงินตั๋วมวยไทยด้วย QR Code หรือบัตรเครดิต ปลอดภัย รวดเร็ว ได้ตั๋วทันทีหลังชำระเงินสำเร็จ'
          : 'Pay for your Muay Thai tickets with QR Code or credit card. Secure, fast, and instant ticket confirmation after payment.';
      } else if (currentPath.includes('/success')) {
        title = language === 'th'
          ? `ชำระเงินสำเร็จ - ${DOCUMENT_TITLE_BRAND_SUFFIX}`
          : `Payment Successful - ${DOCUMENT_TITLE_BRAND_SUFFIX}`;
        description = language === 'th'
          ? 'การชำระเงินสำเร็จแล้ว ตั๋วมวยไทยของคุณถูกยืนยันแล้ว กรุณาตรวจสอบอีเมลสำหรับตั๋วอิเล็กทรอนิกส์'
          : 'Payment successful! Your Muay Thai tickets have been confirmed. Please check your email for e-tickets.';
      } else {
        // Home page - use default
        title = HOME_PAGE_TITLE;
        description = language === 'th'
          ? 'ตั๋วมวยไทยออนไลน์ในประเทศไทย จองง่าย ชำระเงินปลอดภัย ที่นั่งอย่างเป็นทางการสำหรับสนามมวยและงานแข่งขันชั้นนำ'
          : 'Muay Thai tickets online in Thailand. Easy booking, secure payment, and official seats for top stadiums and fight events.';
      }
  ...
  }, [heroImage, language, bookingStep]);
  ...
  // Load ticket config only when stadium is selected (reduces initial API calls from 4+ to 0)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      {/* Stadium buttons - before Fight Highlights */}
      {dbLoaded && stadiums.length > 0 && (
        <StadiumButtonsSection stadiums={stadiums} language={language} t={t} />
      )}

      {/* Up Coming Fight Banner - show before Highlights if configured and visible */}
      {dbLoaded && (() => {
        const banner = upcomingFightBanners.find(b => b.targetId === 'homepage');
        if (!banner || !banner.isVisible || !banner.eventName) return null;
        return <UpcomingFightBanner banner={banner} />;
      })()}
      ...
      {/* Upcoming Fights Section - ซ่อนบนหน้าแรก เพราะแสดงในหน้าของแต่ละสนามแล้ว */}
