...
// Stadium highlights operations (per-stadium fight highlights)
export const getStadiumHighlights = async (stadiumId = null) => {
  try {
    const params = stadiumId ? { stadiumId } : {};
    const response = await axios.get(`${API_URL}/images/stadium-highlights`, { params });
    return response.data;
  } catch (error) {
    console.error('Error getting stadium highlights:', error);
    return stadiumId ? [] : {};
  }
};

export const updateStadiumHighlights = async (stadiumId, highlightIds) => {
  try {
    const response = await axios.put(`${API_URL}/images/stadium-highlights/${stadiumId}`, {
      highlightIds: Array.isArray(highlightIds) ? highlightIds : []
    });
    return response.data;
  } catch (error) {
    console.error('Error updating stadium highlights:', error);
    return null;
  }
};

// Stadium highlight images (image only, unlimited)
export const getStadiumHighlightImages = async (stadiumId = null) => {
  try {
    const params = stadiumId ? { stadiumId } : {};
    const response = await axios.get(`${API_URL}/images/stadium-highlight-images`, { params });
    return response.data;
  } catch (error) {
    console.error('Error getting stadium highlight images:', error);
    return stadiumId ? [] : {};
  }
};

export const createStadiumHighlightImage = async (stadiumId, image, displayOrder = 0) => {
  try {
    const response = await axios.post(`${API_URL}/images/stadium-highlight-images`, {
      stadiumId,
      image,
      displayOrder
    });
    ...
    console.error('Error creating stadium highlight image:', error);
    return null;
  }
};

export const updateStadiumHighlightImage = async (id, image) => {
  try {
    const response = await axios.put(`${API_URL}/images/stadium-highlight-images/${id}`, { image });
    return response.data;
  } catch (error) {
    console.error('Error updating stadium highlight image:', error);
    return null;
  }
};

export const deleteStadiumHighlightImage = async (id) => {
  try {
    await axios.delete(`${API_URL}/images/stadium-highlight-images/${id}`);
    return true;
  } catch (error) {
    console.error('Error deleting stadium highlight image:', error);
    return false;
  }
};

export const reorderStadiumHighlightImages = async (stadiumId, orderedIds) => {
  try {
    const response = await axios.put(`${API_URL}/images/stadium-highlight-images/reorder/${stadiumId}`, {
      orderedIds: Array.isArray(orderedIds) ? orderedIds : []
    });
    return response.data;
  } catch (error) {
    console.error('Error reordering stadium highlight images:', error);
    return null;
  }
};

// Stadium descriptions (คำบรรยาย)
export const getStadiumDescriptions = async (stadiumId = null) => {
  try {
    const params = stadiumId ? { stadiumId } : {};
    const response = await axios.get(`${API_URL}/images/stadium-descriptions`, { params });
    return response.data;
  } catch (error) {
    console.error('Error getting stadium descriptions:', error);
    return stadiumId ? null : {};
  }
};

export const updateStadiumDescription = async (stadiumId, data) => {
  try {
    const response = await axios.put(`${API_URL}/images/stadium-descriptions/${stadiumId}`, data);
    return response.data;
  } catch (error) {
    console.error('Error updating stadium description:', error);
    return null;
  }
};
...
export const getMatchCards = async () => {
  try {
    const response = await axios.get(`${API_URL}/match-cards`);
    return response.data;
  } catch (error) {
    console.error('Error getting match cards:', error);
    return [];
  }
};

export const createMatchCard = async (cardData) => {
  try {
    const response = await axios.post(`${API_URL}/match-cards`, cardData, getAdminAxiosConfig());
    return response.data;
  } catch (error) {
...
export const updateMatchCard = async (id, cardData) => {
  try {
    const response = await axios.put(`${API_URL}/match-cards/${id}`, cardData, getAdminAxiosConfig());
    return response.data;
  } catch (error) {
...
export const deleteMatchCard = async (id) => {
  try {
    await axios.delete(`${API_URL}/match-cards/${id}`, getAdminAxiosConfig());
    return true;
  } catch (error) {
...
export const getMatchCardTickets = async (matchCardId) => {
  try {
    const response = await axios.get(`${API_URL}/match-cards/${matchCardId}/tickets`);
    return response.data;
  } catch (error) {
    console.error('Error getting match card tickets:', error);
    return [];
  }
};

export const createMatchCardTicket = async (matchCardId, ticketData) => {
  try {
    const response = await axios.post(`${API_URL}/match-cards/${matchCardId}/tickets`, ticketData, getAdminAxiosConfig());
    return response.data;
  } catch (error) {
    console.error('Error creating match card ticket:', error);
    return null;
  }
};

export const updateMatchCardTicket = async (ticketId, ticketData) => {
  try {
    const response = await axios.put(`${API_URL}/match-cards/tickets/${ticketId}`, ticketData, getAdminAxiosConfig());
    return response.data;
  } catch (error) {
    console.error('Error updating match card ticket:', error);
    return null;
  }
};

export const deleteMatchCardTicket = async (ticketId) => {
  try {
    await axios.delete(`${API_URL}/match-cards/tickets/${ticketId}`, getAdminAxiosConfig());
    return true;
  } catch (error) {
    console.error('Error deleting match card ticket:', error);
    return false;
  }
};

// Upcoming Fight Banners
export const getUpcomingFightBanners = async () => {
  try {
    const response = await axios.get(`${API_URL}/upcoming-fight-banners`);
    return response.data;
  } catch (error) {
    console.error('Error getting upcoming fight banners:', error);
    return [];
  }
};

export const getUpcomingFightBanner = async (targetId) => {
  try {
    const response = await axios.get(`${API_URL}/upcoming-fight-banners/${targetId}`);
    return response.data;
  } catch (error) {
    console.error('Error getting upcoming fight banner:', error);
    return null;
  }
};

export const clearUpcomingFightBanner = async (targetId) => {
  try {
    await axios.delete(`${API_URL}/upcoming-fight-banners/${targetId}`, getAdminAxiosConfig());
    return true;
  } catch (error) {
    console.error('Error clearing upcoming fight banner:', error);
    return false;
  }
};

export const upsertUpcomingFightBanner = async (targetId, { matchCardId, countdownTo, isVisible, isAuto, eventName, eventDate, eventImage, eventStadiumId }) => {
  try {
    const response = await axios.put(
      `${API_URL}/upcoming-fight-banners/${targetId}`,
      { matchCardId, countdownTo, isVisible, isAuto, eventName, eventDate, eventImage, eventStadiumId },
      getAdminAxiosConfig()
    );
    return response.data;
  } catch (error) {
    console.error('Error upserting upcoming fight banner:', error);
    return null;
  }
};
