...
// Test data - Admin booking notification (payment complete)
...
async function testEmailService() {
  ...
  // Test 1: Send admin notification (payment complete - Pay Solutions/QR)
  console.log('📤 Step 2: Testing Admin Booking Notification (ชำระเงินแล้ว - QR/PromptPay)...');
  try {
  ...
  // Test 1b: Send admin Stripe notification (Credit Card)
  console.log('📤 Step 2b: Testing Admin Stripe Confirmation (ชำระเงินแล้ว - Credit Card)...');
  try {
    const stripeInfo = { sessionId: 'cs_test_abc123', paymentIntentId: 'pi_test_xyz789' };
    const stripeResult = await emailService.sendAdminStripeConfirmation(testBookingData, stripeInfo);
    console.log('✅ Admin Stripe confirmation email sent successfully!');
    console.log('   Message ID:', stripeResult.messageId);
  } catch (error) {
    console.error('❌ Failed to send admin Stripe email:', error.message);
  }
  console.log('');
