...
import db, { 
  ...
  updateHighlight,
  deleteHighlight,
  getStadiumHighlights,
  updateStadiumHighlights,
  getStadiumHighlightImages,
  createStadiumHighlightImage,
  updateStadiumHighlightImage,
  deleteStadiumHighlightImage,
  reorderStadiumHighlightImages,
  getStadiumDescriptions,
  upsertStadiumDescription,
  getStadiumsExtended,
  ...
  updateMatchCard,
  deleteMatchCard,
  deleteExpiredMatchCards,
  getMatchCardTickets,
  createMatchCardTicket,
  updateMatchCardTicket,
  deleteMatchCardTicket,
  deductMatchCardTicketQuantity,
  getUpcomingFightBanners,
  getUpcomingFightBanner,
  upsertUpcomingFightBanner,
  deleteUpcomingFightBanner
} from './database.js';
...
import { calculateBookingPrice } from './services/priceCalculationService.js';

// Import authentication middleware
import { verifyApiKey, requireAdmin, rateLimit, rateLimitEmailVerification } from './middleware/auth.js';
...
app.post('/api/payments/stripe-webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // Send admin notification - Stripe (Credit Card) specific
      const ref = session.metadata?.reference_no;
      if (ref) {
        try {
          const payment = paymentService.getPaymentByReference(ref);
          ...
          const emailData = {
            ...
            stadiumName: resolveName(bookingMetadata?.stadiumData?.name) || booking?.stadium || 'N/A',
            date: bookingMetadata?.dateDisplay || bookingMetadata?.date || booking?.date || 'N/A',
            ticketName: resolveName(bookingMetadata?.ticketData?.name) || booking?.ticket_name || 'N/A',
            zoneName: resolveName(bookingMetadata?.zoneData?.name) || booking?.zone_name || 'N/A',
            quantity: bookingMetadata?.quantity || booking?.quantity || 1,
            totalPrice: payment.amount,
            referenceNo: ref,
            orderNo: payment.id ? String(payment.id).padStart(6, '0') : 'N/A'
          };
          const stripeInfo = {
            sessionId: session.id || null,
            paymentIntentId: session.payment_intent || null
          };
          await emailService.sendAdminStripeConfirmation(emailData, stripeInfo);
          console.log('[Stripe Webhook] Admin Stripe confirmation email sent');

          try {
            await telegramService.sendCreditCardPaymentNotification(emailData, stripeInfo);
            console.log('[Stripe Webhook] Telegram notification sent (Credit Card)');
          } catch (telegramErr) {
            console.warn('[Stripe Webhook] Telegram notification skipped:', telegramErr.message);
          }

          // Send customer confirmation email (same as QR code flow)
          try {
            await emailService.sendCustomerConfirmation(emailData);
            console.log('[Stripe Webhook] Customer confirmation email sent to:', emailData.customerEmail);
          } catch (customerEmailErr) {
            console.error('[Stripe Webhook] Customer email error:', customerEmailErr.message);
          }
        } catch (emailErr) {
...
// Verify email
// ✅ Use custom rate limit that uses email from token instead of IP
// This prevents issues with shared IPs (NAT, proxy, etc.)
app.post('/api/verify-email', rateLimitEmailVerification(6, 3600000), async (req, res) => {
  try {
    ...
    try {
      ...
      // Parse booking data
      let bookingData = verification.booking_data;

      // ✅ Recalculate price from database to ensure accuracy (including daily adjustments)
      if (bookingData.ticketId && bookingData.ticketType && bookingData.quantity) {
        try {
          const priceResult = calculateBookingPrice({
            ...
            date: bookingData.date // ✅ Include date for daily price adjustments
          });

          if (priceResult.success) {
            // Update booking data with correct price from database
            bookingData.totalPrice = priceResult.totalPrice;
            console.log('[Server] ✅ Price recalculated from database:', {
              oldPrice: verification.booking_data.totalPrice,
              newPrice: priceResult.totalPrice,
              unitPrice: priceResult.unitPrice,
              quantity: priceResult.quantity
            });
          } else {
            console.warn('[Server] ⚠️ Failed to recalculate price:', priceResult.error);
            // Keep original price if calculation fails
          }
        } catch (priceError) {
          console.error('[Server] ❌ Error recalculating price:', priceError);
          // Keep original price if calculation fails
        }
      }
...
app.post('/api/bookings', (req, res) => {
  ...
  if (ticketId && ticketType && paymentStatus === 'verified') {
    ...
    let success;
    if (ticketType === 'matchcard') {
      success = deductMatchCardTicketQuantity(ticketId, requestedQuantity);
    } else {
      // ส่ง date ไปด้วยเพื่อแยกจำนวนตั๋วตามวัน
      success = deductTicketQuantity(stadium, ticketId, ticketType, requestedQuantity, date);
    }
...
app.post('/api/payments/create-stripe-checkout', async (req, res) => {
  try {
    const bookingData = req.body;
    ...
    const result = await paymentService.createStripeCheckout(bookingData);
    ...
    if (ref && isSuccess) {
      try {
        ...
        try {
          ...
          try {
            await telegramService.sendQRCodePaymentNotification(emailData);
            console.log('[Webhook] ✅ Telegram notification sent (QR Code)');
          } catch (telegramError) {
            console.warn('[Webhook] Telegram notification skipped:', telegramError.message);
          }
...
app.post('/api/images/highlights', async (req, res) => {
    ...
    clearCache('/api/images/stadium-highlights');
  try {
...
// Stadium Highlights (per-stadium fight highlights)
app.get('/api/images/stadium-highlights', cacheMiddleware('/api/images/stadium-highlights', 5 * 60 * 1000), (req, res) => {
  try {
    const stadiumId = req.query.stadiumId;
    const data = getStadiumHighlights(stadiumId || null);
    res.json(data);
  } catch (err) {
    console.error('Error getting stadium highlights:', err);
    res.status(500).json({ error: 'Failed to get stadium highlights' });
  }
});

app.put('/api/images/stadium-highlights/:stadiumId', async (req, res) => {
  try {
    clearCache('/api/images/stadium-highlights');
    clearCache('/api/images/stadium-highlight-images');
    const stadiumId = req.params.stadiumId;
    const { highlightIds } = req.body;
    const ids = Array.isArray(highlightIds) ? highlightIds : [];
    const result = updateStadiumHighlights(stadiumId, ids);
    res.json(result);
  } catch (err) {
    console.error('Error updating stadium highlights:', err);
    res.status(500).json({ error: 'Failed to update stadium highlights', message: err.message });
  }
});

// Stadium Highlight Images (image only, unlimited)
app.get('/api/images/stadium-highlight-images', cacheMiddleware('/api/images/stadium-highlight-images', 5 * 60 * 1000), (req, res) => {
  try {
    const stadiumId = req.query.stadiumId;
    const data = getStadiumHighlightImages(stadiumId || null);
    res.json(data);
  } catch (err) {
    console.error('Error getting stadium highlight images:', err);
    res.status(500).json({ error: 'Failed to get stadium highlight images' });
  }
});

app.post('/api/images/stadium-highlight-images', async (req, res) => {
  try {
    clearCache('/api/images/stadium-highlight-images');
    const { stadiumId, image, displayOrder } = req.body;
    if (!stadiumId || !image) {
      return res.status(400).json({ error: 'stadiumId and image are required' });
    }
    let processedImage = image;
    if (image && !image.startsWith('http')) {
      processedImage = await processImage(image, { quality: 85, exactWidth: 1500, exactHeight: 405 });
    }
    const id = createStadiumHighlightImage(stadiumId, processedImage, displayOrder ?? 0);
    res.status(201).json({ id, stadiumId, image: processedImage, displayOrder: displayOrder ?? 0 });
  } catch (err) {
    console.error('Error creating stadium highlight image:', err);
    res.status(500).json({ error: 'Failed to create stadium highlight image', message: err.message });
  }
});

app.put('/api/images/stadium-highlight-images/:id', async (req, res) => {
  try {
    clearCache('/api/images/stadium-highlight-images');
    const id = parseInt(req.params.id);
    ...
    updateStadiumHighlightImage(id, processedImage);
    res.json({ id, image: processedImage });
  } catch (err) {
    console.error('Error updating stadium highlight image:', err);
    res.status(500).json({ error: 'Failed to update stadium highlight image', message: err.message });
  }
});

app.delete('/api/images/stadium-highlight-images/:id', (req, res) => {
  try {
    clearCache('/api/images/stadium-highlight-images');
    const id = parseInt(req.params.id);
    deleteStadiumHighlightImage(id);
    res.json({ message: 'Stadium highlight image deleted' });
  } catch (err) {
    console.error('Error deleting stadium highlight image:', err);
    res.status(500).json({ error: 'Failed to delete stadium highlight image' });
  }
});

app.put('/api/images/stadium-highlight-images/reorder/:stadiumId', (req, res) => {
  try {
    clearCache('/api/images/stadium-highlight-images');
    const stadiumId = req.params.stadiumId;
    const { orderedIds } = req.body;
    const ids = Array.isArray(orderedIds) ? orderedIds : [];
    const result = reorderStadiumHighlightImages(stadiumId, ids);
    res.json(result);
  } catch (err) {
    console.error('Error reordering stadium highlight images:', err);
    res.status(500).json({ error: 'Failed to reorder stadium highlight images', message: err.message });
  }
});

// Stadium Descriptions (คำบรรยาย)
app.get('/api/images/stadium-descriptions', cacheMiddleware('/api/images/stadium-descriptions', 5 * 60 * 1000), (req, res) => {
  try {
    const stadiumId = req.query.stadiumId || null;
    const data = getStadiumDescriptions(stadiumId);
    res.json(data);
  } catch (err) {
    console.error('Error getting stadium descriptions:', err);
    res.status(500).json({ error: 'Failed to get stadium descriptions' });
  }
});

app.put('/api/images/stadium-descriptions/:stadiumId', async (req, res) => {
  try {
    clearCache('/api/images/stadium-descriptions');
    clearCache('/api/images/all');
    const stadiumId = req.params.stadiumId;
    const body = req.body;
    const result = upsertStadiumDescription(stadiumId, {
      h1: body.h1,
      h2: body.h2,
      h3: body.h3,
    ...
    console.error('Error updating stadium description:', err);
    res.status(500).json({ error: 'Failed to update stadium description', message: err.message });
  }
});
...
app.get('/api/booking-videos/:stadiumId', (req, res, next) => {
  // Use dynamic cache key that includes stadiumId to prevent cache collision
  const { stadiumId } = req.params;
  const cacheKey = `/api/booking-videos/${stadiumId}`;
  console.log(`[API] Cache key for booking videos: ${cacheKey}`);
  return cacheMiddleware(cacheKey, 5 * 60 * 1000)(req, res, next);
}, (req, res) => {
  try {
    const { stadiumId } = req.params;
...
app.post('/api/booking-videos', requireAdmin, (req, res) => {
  try {
    ...
    // Clear cache for this specific stadium
    clearCache(`/api/booking-videos/${stadium_id}`);
    console.log(`[API] Cleared cache for stadium ${stadium_id} after creating video`);
...
app.put('/api/booking-videos/:id', requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { stadium_id, video_url, title, display_order, is_active } = req.body;
    
    // Get old video data to clear cache for old stadium_id
    const oldVideo = getAllBookingVideos().find(v => v.id === parseInt(id));
    const oldStadiumId = oldVideo?.stadium_id;
    ...
    // Clear cache for both old and new stadium (if stadium_id changed)
    if (oldStadiumId) {
      clearCache(`/api/booking-videos/${oldStadiumId}`);
      console.log(`[API] Cleared cache for old stadium ${oldStadiumId}`);
    }
    const newStadiumId = videoData.stadium_id || stadium_id;
    if (newStadiumId) {
      clearCache(`/api/booking-videos/${newStadiumId}`);
      console.log(`[API] Cleared cache for new stadium ${newStadiumId}`);
    }
...
app.delete('/api/booking-videos/:id', requireAdmin, (req, res) => {
  try {
    ...
    // Get video data before deleting to clear cache for the stadium
    const videoToDelete = getAllBookingVideos().find(v => v.id === parseInt(id));
    const stadiumId = videoToDelete?.stadium_id;
    ...
    // Clear cache for the stadium
    if (stadiumId) {
      clearCache(`/api/booking-videos/${stadiumId}`);
      console.log(`[API] Cleared cache for stadium ${stadiumId} after deleting video`);
    }
...
app.put('/api/images/stadium-schedules/:stadiumId', async (req, res) => {
  try {
    ...
    clearCache('/api/images/stadium-schedules');
    clearCache('/api/images/all');
    res.json(updated);
  } catch (err) {
...
app.post('/api/images/special-matches', async (req, res) => {
  try {
    ...
    clearCache('/api/images/special-matches');
    clearCache('/api/images/all');
...
app.post('/api/images/daily-images', async (req, res) => {
  try {
    ...
    clearCache('/api/images/daily-images');
    clearCache('/api/images/all');
...
app.get('/api/images/all', (req, res) => {
  try {
    ...
    const dailyImages = getDailyImages();
    ...
    const stadiumDescriptions = getStadiumDescriptions();
...
// ============================================================
// Match Cards API (การ์ดแมตช์ชั่วคราว)
// ============================================================

app.get('/api/match-cards', (req, res) => {
  try {
    const cards = getMatchCards();
    res.json(cards);
  } catch (err) {
    console.error('Error getting match cards:', err);
    res.status(500).json({ error: 'Failed to get match cards' });
  }
});

app.get('/api/match-cards/:id', (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const card = getMatchCardById(id);
    if (!card) return res.status(404).json({ error: 'Match card not found' });
    res.json(card);
  } catch (err) {
    console.error('Error getting match card:', err);
    res.status(500).json({ error: 'Failed to get match card' });
  }
});

app.post('/api/match-cards', verifyApiKey, async (req, res) => {
  try {
    const { image, seatingImage, ...rest } = req.body;
    if (!rest.stadiumId || !rest.name || !rest.date) {
      return res.status(400).json({ error: 'stadiumId, name and date are required' });
    }
    let processedImage = image;
    if (image) {
      processedImage = await processImage(image, { quality: 85, maxWidth: 1920, maxHeight: 1080 });
    }
    let processedSeatingImage = seatingImage || null;
    if (seatingImage && seatingImage.startsWith('data:')) {
      processedSeatingImage = await processImage(seatingImage, { quality: 85, maxWidth: 1920, maxHeight: 1080 });
    }
    const card = createMatchCard({ ...rest, image: processedImage || '', seatingImage: processedSeatingImage });
    res.status(201).json(card);
  } catch (err) {
    console.error('Error creating match card:', err);
    res.status(500).json({ error: 'Failed to create match card', message: err.message });
  }
});

app.put('/api/match-cards/:id', verifyApiKey, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { image, seatingImage, ...rest } = req.body;
    let processedImage = image;
    ...
    let processedSeatingImage = seatingImage !== undefined ? (seatingImage || null) : undefined;
    if (seatingImage && seatingImage.startsWith('data:')) {
      processedSeatingImage = await processImage(seatingImage, { quality: 85, maxWidth: 1920, maxHeight: 1080 });
    }
    const card = updateMatchCard(id, { ...rest, image: processedImage || '', seatingImage: processedSeatingImage });
    res.json(card);
  } catch (err) {
    console.error('Error updating match card:', err);
    res.status(500).json({ error: 'Failed to update match card', message: err.message });
  }
});

app.delete('/api/match-cards/:id', verifyApiKey, (req, res) => {
  try {
    const id = parseInt(req.params.id);
    deleteMatchCard(id);
    res.json({ message: 'Match card deleted successfully' });
  } catch (err) {
    console.error('Error deleting match card:', err);
    res.status(500).json({ error: 'Failed to delete match card' });
  }
});

// Match Card Tickets
app.get('/api/match-cards/:id/tickets', (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const tickets = getMatchCardTickets(id);
    res.json(tickets);
  } catch (err) {
    console.error('Error getting match card tickets:', err);
    res.status(500).json({ error: 'Failed to get match card tickets' });
  }
});

app.post('/api/match-cards/:id/tickets', verifyApiKey, (req, res) => {
  try {
    const matchCardId = parseInt(req.params.id);
    const { name, price, quantity } = req.body;
    if (!name || price === undefined) {
      return res.status(400).json({ error: 'name and price are required' });
    }
    const ticket = createMatchCardTicket({ matchCardId, name, price: parseFloat(price), quantity: parseInt(quantity) || 0 });
    res.status(201).json(ticket);
  } catch (err) {
    console.error('Error creating match card ticket:', err);
    res.status(500).json({ error: 'Failed to create match card ticket', message: err.message });
  }
});

app.put('/api/match-cards/tickets/:ticketId', verifyApiKey, (req, res) => {
  try {
    const id = parseInt(req.params.ticketId);
    const { name, price, quantity } = req.body;
    const ticket = updateMatchCardTicket(id, { name, price: parseFloat(price), quantity: parseInt(quantity) || 0 });
    res.json(ticket);
  } catch (err) {
    console.error('Error updating match card ticket:', err);
    res.status(500).json({ error: 'Failed to update match card ticket', message: err.message });
  }
});

app.delete('/api/match-cards/tickets/:ticketId', verifyApiKey, (req, res) => {
  try {
    const id = parseInt(req.params.ticketId);
    deleteMatchCardTicket(id);
    res.json({ message: 'Match card ticket deleted successfully' });
  } catch (err) {
    console.error('Error deleting match card ticket:', err);
    res.status(500).json({ error: 'Failed to delete match card ticket' });
  }
});

// ============================================================
// Upcoming Fight Banners API
// ============================================================

app.get('/api/upcoming-fight-banners', (req, res) => {
  try {
    const banners = getUpcomingFightBanners();
    res.json(banners);
  } catch (err) {
    console.error('Error getting upcoming fight banners:', err);
    res.status(500).json({ error: 'Failed to get upcoming fight banners' });
  }
});

app.get('/api/upcoming-fight-banners/:targetId', (req, res) => {
  try {
    const banner = getUpcomingFightBanner(req.params.targetId);
    if (!banner) return res.json(null);
    res.json(banner);
  } catch (err) {
    console.error('Error getting upcoming fight banner:', err);
    res.status(500).json({ error: 'Failed to get upcoming fight banner' });
  }
});

app.put('/api/upcoming-fight-banners/:targetId', verifyApiKey, (req, res) => {
  try {
    const { matchCardId, countdownTo, isVisible, isAuto, eventName, eventDate, eventImage, eventStadiumId } = req.body;
    const banner = upsertUpcomingFightBanner(req.params.targetId, {
      matchCardId: matchCardId || null,
      countdownTo: countdownTo || null,
      isVisible: isVisible !== false && isVisible !== 0,
      isAuto: !!isAuto,
      eventName: eventName || null,
      eventDate: eventDate || null,
      eventImage: eventImage || null,
      eventStadiumId: eventStadiumId || null
    });
    res.json(banner);
  } catch (err) {
    console.error('Error upserting upcoming fight banner:', err);
    res.status(500).json({ error: 'Failed to save upcoming fight banner' });
  }
});

app.delete('/api/upcoming-fight-banners/:targetId', verifyApiKey, (req, res) => {
  try {
    deleteUpcomingFightBanner(req.params.targetId);
    res.json({ message: 'Banner cleared successfully' });
  } catch (err) {
    console.error('Error deleting upcoming fight banner:', err);
    res.status(500).json({ error: 'Failed to clear banner' });
  }
});
...
(async () => {
  ...
  console.log('✅ Database and migrations ready');
    ...
    // Cleanup expired match cards on startup
    try {
      const deletedCards = deleteExpiredMatchCards();
      if (deletedCards > 0) {
        console.log(`✅ Deleted ${deletedCards} expired match card(s)`);
      }
    } catch (err) {
      console.error('⚠️  Error cleaning up expired match cards:', err);
    }

    // Schedule periodic cleanup of expired match cards every hour
    setInterval(() => {
      try {
        const deleted = deleteExpiredMatchCards();
        if (deleted > 0) {
          console.log(`🗑️  Auto-deleted ${deleted} expired match card(s)`);
        }
      } catch (err) {
        console.error('⚠️  Error in match card cleanup scheduler:', err);
      }
