...
import { getRegularTickets, getSpecialTickets, getTicketQuantityByDate } from '../database.js';
...
export const hasAvailableTickets = (dateString, stadiumId, ticketConfig) => {
  ...
  // Check if tickets can be purchased for this date (time restriction: 17:00 cutoff for today)
  if (!canPurchaseTicketsForDate(dateString)) {
    return false; // Cannot purchase tickets for today after 17:00 Thailand time
  }
...
export const getAvailableTicketsForDate = (stadiumId, dateString) => {
  ...
  if (!canPurchaseTicketsForDate(dateString)) {
    return {
      regularTickets: [],
      specialTickets: []
    }; // Cannot purchase tickets for today after 17:00 Thailand time
  }
