...
import { getStadiumImageSchedules, getSpecialMatches } from '../database.js';
 ...
 */
const getEventName = (stadiumId, dayOfWeek, dateStr, stadiumImageSchedules, specialMatches) => {
 ...
 */
const getEventImage = (stadiumId, dateStr, dayOfWeek, index, stadiumImageSchedules, specialMatches) => {
  // First, check for special matches (exact date match)
  if (specialMatches && specialMatches.length > 0 && dateStr) {
    const specialMatch = specialMatches.find(match => 
      match.stadiumId === stadiumId && match.date === dateStr
...
export const getStadiumEvents = (stadiumId, scheduleDays = [0, 1, 2, 3, 4, 5, 6], limit = 4) => {
  ...
  const specialMatches = getSpecialMatches(stadiumId);
  ...
  for (let i = 0; i <= 28 && upcomingDates.length < limit; i++) {
    ...
    if (scheduleDays.includes(dayOfWeek)) {
      ...
      const eventName = getEventName(stadiumId, dayOfWeek, dateStr, stadiumImageSchedules, specialMatches);
      ...
      events.push({
        ...
        name: eventName,
        date: dateStr,
        image: getEventImage(stadiumId, dateStr, dayOfWeek, upcomingDates.length, stadiumImageSchedules, specialMatches),
        price: null,
        priceRange: null,
        stadiumId: stadiumId,
        isSoldOut: isSoldOut
