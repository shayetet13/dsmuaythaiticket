...
class PaymentService {
  ...
  async createPayment(bookingData) {
    try {
      ...
      if (bookingData.ticketId && bookingData.ticketType) {
        // Calculate price from ticket data (including daily adjustments)
  ...
  async checkPaymentStatusFromAPI(referenceNo) {
    try {
      ...
      // Check if payment has expired (based on expire_date) - only for QR payments that have expire_date
      if (payment.status === 'pending' && payment.expire_date && this.isPaymentExpired(payment.expire_date)) {
   ...
   * Uses same price calculation as QR Code - pulls amount from selected ticket
   */
  async createStripeCheckout(bookingData) {
    ...
    const frontendUrl = (process.env.FRONTEND_URL || 'https://dsmuaythaiticket.com').replace(/\/$/, '');
    // Stripe URL limit: 2048 chars - ensure we don't exceed
    if (frontendUrl.length > 2000) {
      throw new Error('FRONTEND_URL is too long for Stripe (max 2048 chars)');
    }
    ...
    try {
      ...
      // ✅ SECURITY: Calculate price server-side (same as QR Code)
      let totalPrice = 0;
      ...
      const bookingMetadata = {
        stadium: bookingData.stadium || bookingData.stadiumId,
        date: bookingData.date,
        dateDisplay: bookingData.dateDisplay || null,
        zone: bookingData.zone || null,
        ticketId: bookingData.ticketId || null,
        ticketType: bookingData.ticketType || null,
        quantity: bookingData.quantity || 1,
      ...
      // Save payment to database (pending - will be updated by webhook)
      const payment = this.savePayment({
        orderNo: 'STRIPE-' + referenceNo,
        referenceNo: referenceNo,
        amount: totalPrice,
        qrCodeImage: null,
        expireDate: null,
      ...
      // Stripe limits URLs to 2048 chars - stadium images are often base64 (very long), so omit them
      const productData = {
        name: (productDetail || 'Muay Thai Ticket').substring(0, 500),
        description: (bookingData.stadiumData?.name
          ? `${bookingData.stadiumData.name} - ${bookingData.dateDisplay || bookingData.date}`
          : 'Muay Thai Stadium Ticket').substring(0, 500)
      };
      // Only add image if it's a short URL (not base64/data URL)
      const imageUrl = bookingData.stadiumData?.image;
      if (imageUrl && typeof imageUrl === 'string' && imageUrl.startsWith('http') && imageUrl.length <= 2048) {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `${frontendUrl}/success?ref=${referenceNo}`,
      ...
      console.error('[PaymentService] Stripe checkout error:', error);
      throw new Error(`Stripe checkout failed: ${error.message}`);
    }
  }

  /**
   * Handle Stripe webhook - update payment status and create booking
   */
  async handleStripeWebhook(session) {
    const referenceNo = session.metadata?.reference_no;
    if (!referenceNo) {
      console.error('[PaymentService] Stripe webhook: No reference_no in metadata');
      return null;
    }
    ...
    try {
      ...
      if (payment.status === 'paid' || payment.status === 'completed') {
        console.log('[PaymentService] Stripe: Payment already processed:', referenceNo);
        return payment;
      }

      this.updatePaymentStatus(referenceNo, 'paid');
      return this.getPaymentByReference(referenceNo);
    } catch (error) {
      console.error('[PaymentService] Stripe webhook error:', error);
      throw error;
    }
  }
}
