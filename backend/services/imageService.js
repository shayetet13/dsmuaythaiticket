...
export const convertBase64ToWebP = async (base64Image, options = {}) => {
  try {
    const { quality = 85, maxWidth = 1920, maxHeight = 1080, exactWidth, exactHeight } = options;
    ...
    // Exact size with contain (fit - scale to fit, add letterboxing) - for stadium highlights 1500x405
    // ไม่ใช้ withoutEnlargement เพราะจะทำให้ output ไม่ได้ 1500x405 (Sharp จะ clamp เป็นขนาด input)
...
export const processImage = async (image, options = {}) => {
  ...
  // If it's an external URL, return as-is (don't process - we don't own the image)
  ...
  if (image.startsWith('data:image/')) {
    // Skip if already WebP with exact size (client may have pre-processed)
    if (isWebP(image) && !options.exactWidth && !options.exactHeight) {
      return image;
    }
    // Convert to WebP (with optional exact size)
