...
export const initDatabase = async () => {
  ...
  const insertHero = db.prepare(`
    ...
    VALUES (1, '/images/hero/_RMS0100.jpg', 'Muay Thai', '/images/highlights/World class fighters.jpg')
  `);
  insertHero.run();
  ...
  const insertBackground = db.prepare(`
    ...
    VALUES (1, '/images/upcoming-fights-bg.jpg', '/images/highlights/World class fighters.jpg')
  `);
...
export const getHeroImage = () => {
  ...
  const hero = stmt.get();
  return hero || { id: 1, image: '/images/hero/_RMS0100.jpg', alt: 'Muay Thai', fallback: '/images/highlights/World class fighters.jpg' };
};
...
export const updateHeroImage = (image, alt, fallback) => {
  ...
  `);
  stmt.run(image, alt || 'Muay Thai', fallback || '/images/highlights/World class fighters.jpg');
  return getHeroImage();
};
...
export const updateUpcomingFightsBackground = (image, fallback) => {
  const stmt = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      updated_at = CURRENT_TIMESTAMP
  `);
  stmt.run(image, fallback || '/images/highlights/World class fighters.jpg');
