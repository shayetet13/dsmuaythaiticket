...
export const initDatabase = async () => {
  ...
  // Create match_cards table - การ์ดแมตช์ชั่วคราวที่จะถูกลบอัตโนมัติหลังวันที่กำหนด
  db.exec(`
    CREATE TABLE IF NOT EXISTS match_cards (
      ...
      date TEXT NOT NULL,
      image TEXT NOT NULL,
      show_before INTEGER NOT NULL DEFAULT 0,
  ...
  // Add show_before / is_special / seating_image columns if upgrading from older schema
  try { db.exec(`ALTER TABLE match_cards ADD COLUMN show_before INTEGER NOT NULL DEFAULT 0`); } catch (e) {}
  try { db.exec(`ALTER TABLE match_cards ADD COLUMN is_special INTEGER NOT NULL DEFAULT 0`); } catch (e) {}
  try { db.exec(`ALTER TABLE match_cards ADD COLUMN seating_image TEXT`); } catch (e) {}

  // Create match_card_tickets table - ราคาตั๋วสำหรับการ์ดแมตช์
  db.exec(`
    CREATE TABLE IF NOT EXISTS match_card_tickets (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (match_card_id) REFERENCES match_cards(id) ON DELETE CASCADE
    )
  `);
  ...
  // Create upcoming_fight_banners table - แบนเนอร์ Up Coming Fight สำหรับหน้าแรกและแต่ละสนาม
  db.exec(`
    CREATE TABLE IF NOT EXISTS upcoming_fight_banners (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      event_date TEXT,
      event_image TEXT,
      event_stadium_id TEXT,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (match_card_id) REFERENCES match_cards(id) ON DELETE SET NULL
    )
  `);
  // Add new columns to existing table (migration)
  try { db.exec(`ALTER TABLE upcoming_fight_banners ADD COLUMN event_name TEXT`); } catch (e) {}
  try { db.exec(`ALTER TABLE upcoming_fight_banners ADD COLUMN event_date TEXT`); } catch (e) {}
  try { db.exec(`ALTER TABLE upcoming_fight_banners ADD COLUMN event_image TEXT`); } catch (e) {}
  try { db.exec(`ALTER TABLE upcoming_fight_banners ADD COLUMN event_stadium_id TEXT`); } catch (e) {}
  try { db.exec(`ALTER TABLE upcoming_fight_banners ADD COLUMN is_auto INTEGER NOT NULL DEFAULT 0`); } catch (e) {}
  ...
  // Create stadium_highlights table - สำหรับเลือก fight highlights แยกตามสนาม
  db.exec(`
  ...
  // Create stadium_highlight_images table - ภาพอย่างเดียว ไม่จำกัดจำนวน
  db.exec(`
  ...
  // Create stadium_descriptions table - คำบรรยายสนาม (h1, h2, h3, paragraphs, styling, images)
  db.exec(`
  ...
  `);
  try {
    const checkDesc = db.prepare('SELECT 1 FROM stadium_descriptions WHERE stadium_id = ?').get('rajadamnern');
    if (!checkDesc) {
      db.prepare(`
        INSERT INTO stadium_descriptions (stadium_id, h1, h2, h3, paragraphs, font_size, font_family, font_color, background_color, images)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        'rajadamnern',
...
// Stadium highlights operations (per-stadium fight highlights selection)
export const getStadiumHighlights = (stadiumId = null) => {
  if (stadiumId) {
    const stmt = db.prepare(`
      SELECT sh.highlight_id, sh.display_order
      FROM stadium_highlights sh
      WHERE sh.stadium_id = ?
      ORDER BY sh.display_order ASC, sh.id ASC
    `);
    const rows = stmt.all(stadiumId);
    return rows.map(r => r.highlight_id);
  }
  const stmt = db.prepare(`
    SELECT stadium_id, highlight_id, display_order
    FROM stadium_highlights
    ORDER BY stadium_id, display_order ASC, id ASC
  `);
  const rows = stmt.all();
  const byStadium = {};
  rows.forEach(r => {
    if (!byStadium[r.stadium_id]) byStadium[r.stadium_id] = [];
    byStadium[r.stadium_id].push(r.highlight_id);
  });
  return byStadium;
};

export const updateStadiumHighlights = (stadiumId, highlightIds) => {
  const deleteStmt = db.prepare('DELETE FROM stadium_highlights WHERE stadium_id = ?');
  ...
  const insertStmt = db.prepare(`
    INSERT INTO stadium_highlights (stadium_id, highlight_id, display_order)
    VALUES (?, ?, ?)
  `);

  highlightIds.forEach((highlightId, index) => {
    insertStmt.run(stadiumId, highlightId, index);
  });

  return getStadiumHighlights(stadiumId);
};

// Stadium highlight images (image only, unlimited per stadium)
export const getStadiumHighlightImages = (stadiumId = null) => {
  if (stadiumId) {
    const stmt = db.prepare(`
      SELECT id, stadium_id, image, display_order
      FROM stadium_highlight_images
      WHERE stadium_id = ?
  ...
  rows.forEach(r => {
    if (!byStadium[r.stadium_id]) byStadium[r.stadium_id] = [];
    byStadium[r.stadium_id].push({ id: r.id, image: r.image, display_order: r.display_order });
  });
  return byStadium;
};

export const createStadiumHighlightImage = (stadiumId, image, displayOrder = 0) => {
  const stmt = db.prepare(`
    INSERT INTO stadium_highlight_images (stadium_id, image, display_order)
    VALUES (?, ?, ?)
  `);
  const result = stmt.run(stadiumId, image, displayOrder);
  return result.lastInsertRowid;
};

export const updateStadiumHighlightImage = (id, image) => {
  const stmt = db.prepare(`
    UPDATE stadium_highlight_images SET image = ? WHERE id = ?
  `);
  stmt.run(image, id);
};

export const deleteStadiumHighlightImage = (id) => {
  const stmt = db.prepare('DELETE FROM stadium_highlight_images WHERE id = ?');
  stmt.run(id);
};

export const reorderStadiumHighlightImages = (stadiumId, orderedIds) => {
  const updateStmt = db.prepare(`
    UPDATE stadium_highlight_images SET display_order = ? WHERE id = ? AND stadium_id = ?
  `);
  orderedIds.forEach((id, index) => {
    updateStmt.run(index, id, stadiumId);
  });
  return getStadiumHighlightImages(stadiumId);
};

// Stadium descriptions (คำบรรยาย) - per-stadium text with styling
export const getStadiumDescriptions = (stadiumId = null) => {
  const stmt = stadiumId
    ? db.prepare('SELECT * FROM stadium_descriptions WHERE stadium_id = ?')
    : db.prepare('SELECT * FROM stadium_descriptions');
  const rows = stadiumId ? [stmt.get(stadiumId)].filter(Boolean) : stmt.all();
  const parse = (r) => {
    if (!r) return null;
    return {
      ...
      h2: r.h2 || '',
      h3: r.h3 || '',
      paragraphs: r.paragraphs ? JSON.parse(r.paragraphs) : [],
      fontSize: r.font_size || '16px',
      fontFamily: r.font_family || 'inherit',
      fontColor: r.font_color || '#d1d5db',
      backgroundColor: r.background_color || '#111827',
      images: r.images ? JSON.parse(r.images) : []
    };
  };
  if (stadiumId) return parse(rows[0]);
  const map = {};
  rows.forEach((r) => {
    const parsed = parse(r);
    if (parsed) map[parsed.stadiumId] = parsed;
  });
  return map;
};

export const upsertStadiumDescription = (stadiumId, data) => {
  const stmt = db.prepare(`
    INSERT INTO stadium_descriptions (stadium_id, h1, h2, h3, paragraphs, font_size, font_family, font_color, background_color, images, updated_at)
  ...
  stmt.run(
    ...
    data.h2 || '',
    data.h3 || '',
    typeof data.paragraphs === 'string' ? data.paragraphs : JSON.stringify(data.paragraphs || []),
    data.fontSize || '16px',
    data.fontFamily || 'inherit',
    data.fontColor || '#d1d5db',
    data.backgroundColor || '#111827',
    typeof data.images === 'string' ? data.images : JSON.stringify(data.images || [])
  );
  return getStadiumDescriptions(stadiumId);
};
...
// Resolve ticket name (handles object {th, en} or string)
const resolveTicketName = (n) => {
  if (!n) return null;
  if (typeof n === 'object') return (n.th || n.en || '').trim() || null;
  return String(n).trim() || null;
};

// Get ticket display name for a booking (stadium_id, ticket_id, ticket_type, date)
export const getTicketNameForBooking = (stadiumId, ticketId, ticketType, date) => {
  if (!stadiumId || !ticketId || !ticketType) return null;
  const cleanDate = date ? String(date).split(':')[0].trim() : null;
  try {
    if (ticketType === 'regular') {
      const stmt = db.prepare('SELECT name FROM regular_tickets WHERE stadium_id = ? AND id = ?');
      const ticket = stmt.get(stadiumId, ticketId);
      if (!ticket) return null;
      const baseName = resolveTicketName(ticket.name);
      if (cleanDate && baseName) {
        return getTicketNameForDate(stadiumId, ticketId, ticketType, cleanDate, baseName) || baseName;
      }
      return baseName;
    }
    if (ticketType === 'special') {
      const stmt = db.prepare('SELECT name FROM special_tickets WHERE stadium_id = ? AND id = ? AND date = ?');
      const ticket = stmt.get(stadiumId, ticketId, cleanDate || '');
      if (!ticket) {
        const fallbackStmt = db.prepare('SELECT name FROM special_tickets WHERE stadium_id = ? AND id = ?');
        const fallback = fallbackStmt.get(stadiumId, ticketId);
        if (!fallback) return null;
        const baseName = resolveTicketName(fallback.name);
        if (cleanDate && baseName) {
          return getTicketNameForDate(stadiumId, ticketId, ticketType, cleanDate, baseName) || baseName;
        }
        return baseName;
    ...
    console.warn('[getTicketNameForBooking] Error:', err.message);
  }
  return null;
};
...
export const getAllBookings = () => {
  ...
  // Merge bookings with payments and enrich with ticket_name
  return bookings.map(booking => {
    ...
    if (booking.ticket_id && booking.ticket_type) {
      ticket_name = getTicketNameForBooking(
        booking.stadium_id,
        booking.ticket_id,
        booking.ticket_type,
        booking.date
      );
      if (!ticket_name && payment?.booking_metadata) {
        try {
          const meta = JSON.parse(payment.booking_metadata);
          ticket_name = resolveTicketName(meta?.ticketData?.name) || null;
        } catch (_) {}
      }
    }
    ...
    return {
      ...booking,
      ticket_name: ticket_name || undefined,
...
// ============================================================
// Match Cards Operations (การ์ดแมตช์ชั่วคราว)
// ============================================================

const mapCard = (c, tickets = []) => ({
  id: c.id,
  stadiumId: c.stadium_id,
  name: c.name,
  date: c.date,
  image: c.image,
  seatingImage: c.seating_image || null,
  showBefore: c.show_before === 1,
  isSpecial: c.is_special === 1,
  createdAt: c.created_at,
  tickets: tickets.map(t => ({
    id: t.id,
    matchCardId: t.match_card_id,
    name: t.name,
    price: t.price,
    quantity: t.quantity
  }))
...
export const getMatchCards = () => {
  const cards = db.prepare('SELECT * FROM match_cards ORDER BY date ASC, created_at ASC').all();
  const ticketStmt = db.prepare('SELECT * FROM match_card_tickets WHERE match_card_id = ? ORDER BY created_at ASC');
  return cards.map(c => mapCard(c, ticketStmt.all(c.id)));
};

export const getMatchCardById = (id) => {
  const c = db.prepare('SELECT * FROM match_cards WHERE id = ?').get(id);
  if (!c) return null;
  const tickets = db.prepare('SELECT * FROM match_card_tickets WHERE match_card_id = ? ORDER BY created_at ASC').all(id);
  return mapCard(c, tickets);
};

export const createMatchCard = (card) => {
  const stmt = db.prepare(`
    INSERT INTO match_cards (stadium_id, name, date, image, seating_image, show_before, is_special)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(
    card.stadiumId, card.name, card.date, card.image,
    card.seatingImage || null,
    card.showBefore ? 1 : 0,
    card.isSpecial ? 1 : 0
  );
  return getMatchCardById(result.lastInsertRowid);
};

export const updateMatchCard = (id, card) => {
  const stmt = db.prepare(`
    UPDATE match_cards
    SET stadium_id = ?, name = ?, date = ?, image = ?, seating_image = ?, show_before = ?, is_special = ?, updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `);
  stmt.run(
    card.stadiumId, card.name, card.date, card.image,
    card.seatingImage !== undefined ? (card.seatingImage || null) : null,
    card.showBefore ? 1 : 0,
    card.isSpecial ? 1 : 0,
    id
  );
  return getMatchCardById(id);
};

export const deleteMatchCard = (id) => {
  const stmt = db.prepare('DELETE FROM match_cards WHERE id = ?');
  return stmt.run(id);
};

export const deleteExpiredMatchCards = () => {
  const today = new Date().toISOString().split('T')[0];
  const stmt = db.prepare('DELETE FROM match_cards WHERE date < ?');
  const result = stmt.run(today);
  return result.changes;
};

// Match Card Tickets
export const getMatchCardTickets = (matchCardId) => {
  const stmt = db.prepare('SELECT * FROM match_card_tickets WHERE match_card_id = ? ORDER BY created_at ASC');
  return stmt.all(matchCardId).map(t => ({
    id: t.id,
    matchCardId: t.match_card_id,
    name: t.name,
    price: t.price,
...
export const createMatchCardTicket = (ticket) => {
  const stmt = db.prepare(`
    INSERT INTO match_card_tickets (match_card_id, name, price, quantity)
    VALUES (?, ?, ?, ?)
  `);
  const result = stmt.run(ticket.matchCardId, ticket.name, ticket.price, ticket.quantity || 0);
  const t = db.prepare('SELECT * FROM match_card_tickets WHERE id = ?').get(result.lastInsertRowid);
  return { id: t.id, matchCardId: t.match_card_id, name: t.name, price: t.price, quantity: t.quantity };
};

export const updateMatchCardTicket = (id, ticket) => {
  const stmt = db.prepare(`
    UPDATE match_card_tickets
    SET name = ?, price = ?, quantity = ?, updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `);
  stmt.run(ticket.name, ticket.price, ticket.quantity, id);
  const t = db.prepare('SELECT * FROM match_card_tickets WHERE id = ?').get(id);
  return { id: t.id, matchCardId: t.match_card_id, name: t.name, price: t.price, quantity: t.quantity };
};

export const deleteMatchCardTicket = (id) => {
  const stmt = db.prepare('DELETE FROM match_card_tickets WHERE id = ?');
  return stmt.run(id);
};

export const deductMatchCardTicketQuantity = (ticketId, quantity) => {
  const ticket = db.prepare('SELECT quantity FROM match_card_tickets WHERE id = ?').get(ticketId);
  if (!ticket || ticket.quantity < quantity) return false;
  const result = db.prepare(`
    UPDATE match_card_tickets SET quantity = quantity - ?, updated_at = CURRENT_TIMESTAMP
    WHERE id = ? AND quantity >= ?
  `).run(quantity, ticketId, quantity);
  return result.changes > 0;
};

// ============================================================
// Upcoming Fight Banners
// ============================================================

const mapBanner = (r) => ({
  id: r.id,
  targetId: r.target_id,
  matchCardId: r.match_card_id,
  countdownTo: r.countdown_to,
  isVisible: r.is_visible === 1,
  isAuto: r.is_auto === 1,
  eventName: r.event_name || null,
  eventDate: r.event_date || null,
  eventImage: r.event_image || null,
  eventStadiumId: r.event_stadium_id || null,
  updatedAt: r.updated_at
});

export const getUpcomingFightBanners = () => {
  const rows = db.prepare('SELECT * FROM upcoming_fight_banners ORDER BY target_id ASC').all();
  return rows.map(mapBanner);
};

export const getUpcomingFightBanner = (targetId) => {
  const r = db.prepare('SELECT * FROM upcoming_fight_banners WHERE target_id = ?').get(targetId);
  if (!r) return null;
  return mapBanner(r);
};

export const upsertUpcomingFightBanner = (targetId, { matchCardId, countdownTo, isVisible, isAuto, eventName, eventDate, eventImage, eventStadiumId }) => {
  db.prepare(`
    INSERT INTO upcoming_fight_banners (target_id, match_card_id, countdown_to, is_visible, is_auto, event_name, event_date, event_image, event_stadium_id, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ON CONFLICT(target_id) DO UPDATE SET
      match_card_id = excluded.match_card_id,
      countdown_to = excluded.countdown_to,
      is_visible = excluded.is_visible,
      is_auto = excluded.is_auto,
      event_name = excluded.event_name,
      event_date = excluded.event_date,
      event_image = excluded.event_image,
      event_stadium_id = excluded.event_stadium_id,
      updated_at = CURRENT_TIMESTAMP
  `).run(
    targetId,
    matchCardId || null,
  ...
  return getUpcomingFightBanner(targetId);
};

export const deleteUpcomingFightBanner = (targetId) => {
  db.prepare('DELETE FROM upcoming_fight_banners WHERE target_id = ?').run(targetId);
};
